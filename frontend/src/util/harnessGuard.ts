 ...
 *
 * ── P105 这一批动的一处（收 P103 问题 #7）────────────────────────────────
 *  · **C（`onWarning`）**：从 `blocked` 挪到 `rounds`，挪的**只有记账那一半**。
 *    P103 把它拦掉之后留下一句：**「这一轮少了哪个能力」在界面上没有第二个出处了**
 *    （轮次卡上没有这一格）。这一批把那一格加上了（`AgentActivity` 的
 *    `round.warnings` + 那一条「本轮少了这几个能力」），于是：
 *    **记**（`writeRounds(noteId, … warnings …)`，排在 guard 前面，切走照记）
 *    和**弹**（那句红字 toast，排在 guard 后面，切走一个字不弹）**各判各的**。
 *
 ...
 *    「A 的内容写进了 B」当场长回来。
 *  · ~~**弹 toast 那一条**（`onWarning`）~~ ——**P105 C 把它挪走了**，
 *    但挪的**只有记账那一半**：`writeRounds(noteId, … warnings …)` 排在自己那句
 *    guard 前面（这一轮少了哪个能力是**这一篇**的账），那句红字 toast 照旧排在
 *    guard 后面 ⇒ **切走之后一个字都不弹**。P103 B ③ 判的「照拦」说的是**弹**，
 *    这一条一个字没动；变的是「记」——P103 问题 #7 逐字写着
 *    「要接就得先给轮次卡加一格『这一轮少了什么』」，那一格在 `AgentActivity` 里。
 *
 * ── 它答不了什么 ────────────────────────────────────────────────────────
...
export const HARNESS_GUARD: Record<string, GuardEntry> = {
  ...
  // ⚠️ **它在 `rounds` 这一档，但「弹不弹」那一半没变**（P105 C）：
  //    `writeRounds(noteId, … warnings …)` 记这一轮少了哪个能力（那是这一篇的账，切走照记），
  //    而那句红字 toast 照旧排在自己那句 guard **后面**——切走之后**一个字都不弹**。
  //    P103 B ③ 判的「照拦」说的是**弹**，这一批加的是**记**，两件事。
  onWarning: { verdict: 'rounds', why: '`writeRounds(noteId, … warnings …)` 记这一轮少了哪个能力（P105 C：轮次卡上那一格）；那句红字 toast 照旧排在 guard 后面，切走之后一个字都不弹（一轮能来好几条，是噪声不是通知）' },
