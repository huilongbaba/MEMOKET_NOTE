...
@pytest.mark.parametrize("text,why", [
    ...
    # **「前面没有空白」原来在这张表里，P55 #1 把它挪走了**（挪到下面那条正例）：
    # P53 实拍「…应把这一步退回文案和设计重新处理。做爰片」句号后面一个空格都没有，
    # 而它一路进了终稿。空白从「必须有」改成「可有可无」，另外两条一个字没动。
    ("日本一本道", "不在句末标点之后"),
    ("应把这一步退回文案和设计重新处理。做爰片，然后继续写这一段剩下的话。", "不在段末（后面还有字）"),
])
...
# P53 实拍逐字（`<scratch>/p53/runs/3a3a96354546.json` 第 3 轮 `streamed` 的末尾）。
P55_TAIL = ("工程只负责提供行为数据，不能用“功能已经上线”替代任务完成数据；"
            "销售若发现 demo 仍要额外解释某一步，应把这一步退回文案和设计重新处理。做爰片")


def test_垃圾尾巴_前面没有空白也要认():
    """P55 #1：同一条判据上叠着**两个**洞——词表里没有这个词，而且它前面没有空白。
    只补词表那一个，这条正则照样看不见它。

    量程（两刀各验一次）：
      · 把 `做爰片` 从 `JUNK_WORDS` 里拿掉 → 这条红（词表那一半）；
      · 把 `_TAIL` 里的 `[ \\t\\u3000]*` 改回 `\\s+` → 这条红（形状那一半）。
    """
    assert junk_tails(P55_TAIL) == ["做爰片"]
    fixed = strip_junk_tails(P55_TAIL, junk_tails(P55_TAIL))
    assert fixed.endswith("退回文案和设计重新处理。"), fixed[-20:]
    assert junk_tails(fixed) == [], "修完再判不许还命中（原子 fix 那一支要靠它）"
...
def test_词表只从实拍来():
    ...
    P55 加的第二个词 `做爰片` 的来历：12 批 47 份跑 / 229 轮扫了两遍——形状扫（不带词表）
    全语料只有这两个词；「材料和用户原文里都没有的汉字」765 个逐个读，只有 `爰` 一个异常。
    台账 P55 节里贴了原文和两个脚本。"""
    assert JUNK_WORDS == ("一本道", "做爰片"), f"词表变大了，台账里贴原文了吗：{JUNK_WORDS}"
