...
class RecallOut(BaseModel):
    ...
    #
    # **`[]` 和 `None` 不是一回事**（P46 #1）：`[]` = 判过了，一条都摆不出来
    # （全库 9/765 条落到这一档），前端据此如实说一句、**不许**退回 `terms` 那串更碎的；
    # `None` = 这一趟**没判成**（判据自己抛了 / 老后端没有这一格），那时候退回 `terms` 是对的。
    # 默认 `None`：没填这一格的调用方本来就没判过。
...
class EditOut(BaseModel):
    ...
    # 空 = 模型真的没给建议。
    note: str = ""
    # **「模型说不需要」和「模型没答上来」是两件事**（P37 #4 / P35 #5）。
    #
    # P35 实拍：散文档 / 错键档下「扩展上下文」弹的是「模型认为不需要补充上下文。」
    # ——那是一句**模型从没说过的话**。它是前端那一支的 `emptyNote`，而这个 schema
    # 原来只有「撞长度上限」那一档有值（`compose._truncated_note`），**没有第三种可能**。
    #
    # 判据窄，只认形状：这次回包里**连该有的键都没有**（不是 JSON 对象 / 没有 `text`
    # 或 `before`/`after`）才叫「抽不出」。键在、值是空串的，那是模型按格式答的
    # 「这个方向不用补」（`EXPAND_SYSTEM` 明写着「都不需要就都留空字符串」），
    # **那一档不许算成没答上来**。
    unparsed: bool = False
...
class VerifyOut(BaseModel):
    """校验结果。**空的 `findings` 有三种完全不同的来历，得分得开**（P37 #2 / P35 #3）。

    P35 实拍：terrence 满库（20417 条事实），词法召回**真的回了 1 条**，模型答得不合形状，
    面板上却写着「没有找到能支持或反驳这段内容的记录——不代表内容没问题，只是知识库里
    没有相关信息。」——**说的是假话**：知识库里有相关信息，是模型没按格式答。
    跟 P9 #26 那句「知识库里没有跟这段沾边的记录。——假的」是同一个形状。

    所以多两格，前端 `components/VerifyPanel.tsx` 按它们分三句：

    * `checked` —— 这一次**真的翻过几条记录**（引用 + 词法召回 + 托盘，去重之后）。
      0 才是「知识库里没有相关信息」。
    * `unparsed` —— 模型这次答的**抽不出判断**（不是列表 / 列表里一条都不合格式）。
      有它就不许说「没找到」，那是替模型背锅。
    """

    findings: list[VerifyFinding]
    took_ms: float
    # 这一次交给模型去核的记录条数。0 = 知识库里真的没有沾边的。
    checked: int = 0
    # 模型答的抽不出判断（形状不对），跟「查了、没话说」是两件事。
    unparsed: bool = False
...
class JourneySegment(BaseModel):
    ...
    # **为什么这一段永远不会有描述**（空串 = 还没轮到它，不是补不了）。
    # 盘上本来就有这个字段（`没有截图` / `截图已过期` / `没说出具体的东西`），
    # 只是一直没交给界面——于是界面只能拿 `has_frame` 当它的替身去猜，
    # 而那个替身**会骗人**：第 793 轮（P50）在真实数据上实拍到
    # 09-16 有 37 段、09-17 有 71 段 `skip` 是「没有截图」而 `frames` 还留着一条
    # 早就不存在的路径，于是 `has_frame=true` → 按钮写着「描述这 10 段」→
    # 点下去只回一句「没有要描述的了」，点多少次都一样（P20 清单 #5 那个死胡同）。
    # **判据要守来源**：补不了就是补不了，别让界面去猜。
    skip: str = ""
