...
> ⚠️ **风格来源在第 711 轮换过一次，本文里带 `design-theme.css:行号` /
> `design-detail.css:行号` 的引用都是旧那一版的（0.3.x），当参考读、别当准绳。**
> **准绳是 [`MEMOKET_DESKTOP_0.5.10.md`](MEMOKET_DESKTOP_0.5.10.md)**——
> 用户 mac 上装着的正式版，实拍 + `app.asar` 出货 CSS 两路取证。
> 两份打架时以那一份为准；本文只保留**我们自己的**规矩（令牌怎么接、
> 闸门查什么、有意的偏离）。

- **风格来源**：`/Applications/Memoket.app` **0.5.10**（2026-09-14 构建）。
  它自己又对齐移动端 APP 的 `AppTheme`（品牌紫 `#620FF0`）。
  旧来源（`~/code/memoket_desktop` 仓 `v3-demo-ux` 分支上 2026-06 的展示稿）
  **已作废**——第 679–710 轮照着它调的基调整个偏了，用户一句
  「文件列表栏那个淡紫色太丑」才查出来。**参照物要认版本号，不认目录名。**
- **本应用的起点**：配色和组件原本抄自 Trilium（`frontend/src/shell.css` 是
  Trilium 令牌，`frontend/src/styles.css:1-60` 是一层语义别名）。
  树行的尺寸、选中态的「浮起来的卡片」这些一直是 Trilium 的遗留，
  第 712 轮起逐条换成 0.5.10 的。
- **这次要做的事**：把**视觉身份**换成 memoket 的，**布局和交互不动**（见 §0 铁律）。
...
**大字另走一套字族。** `--display`（`SF Pro Display` 打头）只给大标题用
（0.5.10 的 `.home-greet` 36px 走的就是它，他们把这个令牌叫 `--serif`）。
SF 这两支是给不同尺寸调过的：Text 加粗了细节笔画保证小字清楚，
Display 收紧字距和开口，大字才不显得松——**苹果自己在 20px 上下切换**。
现在接在 `.note-title` 和 `.kb-note-title` 上（第 712 轮）。
...
| | 值 | 源 |
|---|---|---|
| 高度 | `33px`（紧凑场合 28px） | 旧版 `design-theme.css:222`；0.5.10 的导航行是 36，见参照文档 |
| 圆角 | `var(--r-sm)`（9px） | 同 |
| 内边距 | `0 13px` | 同 |
| 字号/字重 | 13.5px / 600 | 同 |
...
| | 值 | 源 |
|---|---|---|
| 浅色选中 | 底 `--mk-surface`，**不投影、不加粗** | 0.5.10 `.nav-item.sel { box-shadow: none }`（第 712 轮实拍逐像素核对） |
...
**按 0.5.10 的 `.home-notescard` 重量过**（第 715 轮）：
底 `--mk-surface`、边 `1px --mk-line`（**`.10`，不是 `.05`**）、
圆角 **`--r-lg`（18）**、内边距 **18**、`--shadow-card`。

**hover 是他们的签名动作**：边线染 **45% 品牌色** + `translateY(-1px)` +
`--shadow-card-hover`。比「底色变一点」清楚得多，而且它顺便回答了「这张卡能不能点」——
所以**只给真能点的卡**（§3 第 15 条）：`.card.clickable` / `a.card` / `button.card`。

（旧值 `.05` / 12 / 12 / 无阴影是第 692 轮从 0.3.x 展示稿上量的，全偏轻一档。）
...
### 2.16 底部 composer（正文的 AI 入口）

**AI 入口在正文底部，不在正文右上角。**（第 714 轮，用户点名原来那一行「很奇怪」）

构造照 0.5.10 的 `.home-ask-dock` + `.home-composer`：

| | 值 |
|---|---|
| dock | 贴底 104px，`pointer-events: none`，`::before` 是遮罩渐变（正文淡出，不是被切） |
| 胶囊 | `min(760px, 100%)`、圆角 `--r-pill`、底 `--mk-surface-glass`、`--shadow-composer`、`backdrop-filter: blur(10px) saturate(145%)` |
| 玻璃边 | `::before` 一条 1px `--glass-edge` + 一道斜向 `--glass-hi` 高光 + `inset 0 1px 0` |
| 圆钮 | 36×36，`--r-pill` |
| 主钮 | **排在最右**（跟他们的发送钮同位）；输入框空着是「续写」，有字变「发送」 |

**两条跟他们不一样、理由要记住的**：

1. **dock 不吃指针事件。** 他们的 dock 自己拿走命中测试（注释：「这块地方视觉上
   盖住了文件行，就得连命中测试一起拿走」）。我们下面是**可编辑的正文**——
   那 100px 里要能点进去定位光标，所以只有胶囊本身 `pointer-events: auto`。
2. **专注模式下跟着正文那一列靠左**，不居中在整个面板上（第 722 轮实拍：
   它会飘到正文右边去）。

**玻璃边和高光两套主题都要给。** 浅色那条 `rgba(255,255,255,.58)` 压在深色底上
是一圈**发光的白描边**（第 715 轮实拍抓到）。加新令牌时两套一起给，别等实拍来抓。
...
17. **按钮的标签不能就是一个符号字形。** `<button>✕</button>`
    ...
    判据**从闭合标签往回认**（`>符号</button>`），不从 `<button` 往后认：
    第 704 轮这条连错两次才立住——先是逐行扫漏掉跨行写的按钮，改成整份文件
    扫之后又栽在 `onClick={(e) => …}` 的箭头上（`[^>]*>` 在那个 `>` 提前收尾）。
    改对之后当场多抓出 5 处漏网的（`…` ×2、`↑` `↓` `✎`）。
...
点完之后界面**不能是静的**。两条路，二选一：

- 自己把主控件变成「停止」（续写 / 智能续写就是这样，`.fb-btn.primary.running`）；
- 或者进 `App.tsx` 的 `BUSY_LABEL`，composer 里那条忙碌条会显示「在做什么」。

**闸门 `check-busy`（第 28 条）两个方向都查**：新加的 `setLoading('x')` 没反馈要报，
`BUSY_LABEL` 里留着没人用的标签也要报。要走第一条路的，得在
`check-busy.mts` 的 `OWN_FEEDBACK` 里**写清楚它的反馈长什么样**——
写不出来就说明它没有。

这条是第 733 轮补的：「做成幻灯片」实测跑 **20 秒**，而 `loading === 'slides'`
只被用来禁菜单项，`api.makeSlides` 的进度回调还被丢成了 `() => {}`。
...
5. **量不出来就别猜：用 `rect:` 探针问 DOM。**
   ...
   **实拍告诉你哪里不对，量才告诉你为什么。**
6. **扫不到东西的闸门会一直是绿的。** 扫描型闸门（遍历 `src/` 找违规的那几条）
   开扫前先声明「我至少该看到这么多文件」，看不到就自爆（`scripts/_corpus.mts`）。
   第 704 轮栽过：符号按钮那条判据前两版都跑绿了，其实一个文件都没匹配上。
   变异验证的做法是**把 `src` 指到一个子目录**，五条闸门都得自爆。
7. **闸门自己也会出错，出错就修闸门**。这一段修过三处：`check-css-classes`
