...
class Checks:
    ...
    def _fix_events(st: State, fired: str, ran: int, verdict) -> list[Event]:
        ...
        **`fix_note` 空着的判据一个字都不说**，两个读者都不写。不是偷懒：
        ...
        比不说更糟。没填的只可能是新加判据时漏了——那种情况保持 P23 那版的老行为
        （静默），由闸去红，而不是在这里发一句半截话。

        ⚠️ **这段抬头原来写着「仓里三处真的 `fix` 都填了 `fix_note`」，那句话是假的**
        （P58 走查 #2 抓到）：静态一扫是 **12 处 `Verdict(... fix=...)`，
        当时只有 5 处填了**，另外 7 处动了用户眼前的正文一个字不说。
        **P59 ② 把那 7 处逐处配上了措辞，现在是 12 / 12。**
        数它的那条闸是 `tests/test_p26.py::test_3_仓里每一处fix_note都得是数出来的`
        （AST 静态扫，不是数人记得住的那几处）。
        """
    ...
    async def after_judge(self, st: State) -> None:
        ...
        **`STUCK_ROUNDS` 卡死放行那一支到今天仍然不写它。** P58 走查 #5 记下这条时
        的理由是「没有能复现的局面」。**P59 ④ 把局面摆出来了**
        （`<scratch>/p59/stuck9.py`，真 `loop.run` + 假打分器），结论**还是不加**——
        但换了三条能查的理由：

        1. **这一支在真跑语料里 0/284 轮开过火**（15 批 / 61 份跑；对照 `JUDGE_FLOOR`
           12 轮、`advisory` 2 轮）。够不着的原因跟 P6 问题 4 记的是同一条：
           `check_streak` 的键是 `dimension\\0message` **带判词原文**，而第一大户
           `citations_present` 的判词里有「这一轮写了 N 字」，每轮都不一样，
           `streak` 永远升不到 3。
        2. **8 个模式里 6 个的 `stop_when` 没有 `check_stuck`**（ANALYSIS / CHART /
           CUSTOM / EDA / PROMPT / TABLE 六个 block 模式）。在那 6 个上把 `complete`
           压回 `continue` 之后**没有任何一条规则接得住**，那一跑会一路跑到
           `max_rounds`——多花的是真模型调用，换来的只有「不说写完了」。
        3. 只在 `NOTE` / `SECTION` 上确实是纯收益，而且**不多跑一轮**：
           同一句判词连响 3 轮 ⊆ 同一条判据在最近 4 轮里响 3 次，所以
           `STUCK_ROUNDS` 一放行，`check_stuck`（`CHECK_STUCK_ROUNDS = 3`）
           必在同一轮成立——实测停机从 `'complete'` 变成 `'check_stuck'`，轮数不变。
           **但判据宁可窄一点**：为两个模式加一行、让另外六个模式多烧轮子不划算。
           真要治，该治的是「那 6 个模式要不要也停 `check_stuck`」，那是另一件事，
           **先量再改**。

        摆出来时顺带钉死的一条：`SHIP_BEST_ON` 那一格在这条路上**永远显不出差别**——
        放行的那一轮必然是这一跑第一个拿到真分的轮（前面几轮被同一条判据短路成
        单维 level=0），它的 `rank` 不可能低于 `st.best`。所以少写这一行的代价
        **只有停机理由和收工那句话**这一项，不含「交错了正文」。
        闸：`tests/test_p59.py::test_4_*`。
