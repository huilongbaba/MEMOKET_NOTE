...
def test_第一条f_小库兜底那句注释不成立(real_corpus):
    ...
    **在 193 unit 的库上它只是把门槛抬到 20/193 = 10.4%，不是不启用**——
    ...
    #
    # ⚠️ **P88 之后 `连接` 又回到这一列**（照实记，跟上面那段并排读，别把上面那句删了）：
    # 新串的那条主语面轴（`kb/topic_face.WhoFace`）判它主语面 **1.043**——
    # 比「从这个库里随手抓同样多条」还宽——于是不再捞回来。
    # `topics` 那条轴当初判它 0.682「不泛」，**两条轴在这一串上意见相反**；
    # 这一刀的规矩是「两条都说不泛才捞」（只做减法），所以听挡的那一条。
    # 另外五个被 P84 捞回来的（`众筹`/`手环`/`设计`/`产品`/`硬件`）**一个都没动**。
    still = [t for t in probes if common(t)]
    assert still == ["工作", "ai", "app", "memory", "2026", "agent", "记录", "使用", "连接"], still
