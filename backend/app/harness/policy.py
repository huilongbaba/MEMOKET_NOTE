...
def adjust(policy: RuntimePolicy, fb: RoundFeedback) -> tuple[RuntimePolicy, list[str]]:
    ...
    # 一点没变，等于让它拿同样的信息再赌一次。
    #
    # **「改写掉那句」只对这一轮新写的说**（P6 问题 1）。P5 实拍：这句 steer 原来
    # 不分是谁写的，打分器把用户开跑前的原文判成「查无此事」→ 这里喊「改写掉」
    # → 修订把「与陈校的沟通从招生策略切入」换成对冲句。用户写的人名 / 日期
    # 不是编的，是材料没摘到；那是留着的，不是改掉的。
    if fb.level("factual_grounding") < 2:
        ...
        steer_parts.append(
            ...
            + (f"：{note}" if note else "")
            + ("。知识库是空的，查不到任何东西："
               "**只对这一轮新写的句子**：把那句改写成不含具体人名/日期/数字的说法，或者直接删掉，"
               "不要保留编造的细节。用户开跑前写好的段落不动。"
               if fb.kb_empty else
               "。这一轮先把这些点查实——查得到就用查到的原话；"
               "查不到的，**只对这一轮新写的句子**改写成不含具体人名/日期/数字的说法，"
               "不要保留编造的细节。用户开跑前写好的段落里的人名/日期/数字不是编的，是材料没摘到，不动。")
        )
