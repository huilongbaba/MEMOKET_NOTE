...
# 还没填的格子。**正文里写着「填写具体金额」的单元格，不是「金额已经写了」。**
#
# P22 #1 实拍两处误翻，两处都栽在这上面：
#   · N5 `603dca25403a` B4「把本月各中介的实际金额、原话、买家反馈、时间范围和证据逐项填入表格」
#     翻成「已写（正文第 7 行起）」，而第 7 行起那张表的格子逐字写着
#     `填写具体金额 | 填写原话或明确判断 | 填写买家类型 | 填写时间范围`——
#     命中的 13 个词元里有 5 个（金额 / 原话 / 买家 / 时间 / 范围）就是从这些占位格里来的。
#   · N3 `c3464ab74c7d` B5 同一个形状（`< X%` / `< Y%` 的阈值占位）。
#
# 只认**前缀**，只认整格 / 整行，不做模糊：写窄一点，误伤比漏报贵。
# 行数和分段一个不动（空出来的位置留一个 `·`），不然行号会飘。
_PLACEHOLDER_CELL = re.compile(r"^\s*(?:填写|待填|待补|待定|待确认|待核实|请填|TBD|N/?A)")


def _strip_placeholders(content: str) -> str:
    """把「还没填的格子」从正文里抹掉，行号和分段保持原样。"""
    out: list[str] = []
    for line in (content or "").split("\n"):
        if "|" in line:
            out.append("|".join("" if _PLACEHOLDER_CELL.match(c) else c for c in line.split("|")))
        elif _PLACEHOLDER_CELL.match(line):
            out.append("·")            # 非空，`_paragraphs` 才不会把一段劈成两段
        else:
            out.append(line)
    return "\n".join(out)


# 「待补」翻成「已写」的门槛。**P24 在 P4（26 条）+ P22（31 条）两批 57 条节拍上重量过**
# （scratch `p24/m1_variant3.py`，零模型，真值逐条读原文判的）。
#
# 原来的 0.30 是只在 P4 那批上量的，P22 复测把它的问题拍了出来：
# 真·已写最低 0.352，真·待补最高 0.387——**两边在旧口径下是重叠的，落在哪个阈值都切不开**。
# 先把占位格剔掉（`_strip_placeholders`），真·待补那一侧塌下来：
#   真·已写 0.165 / 0.352 / 0.357 / 0.375 / 0.385 / 0.458 / 0.464 / 0.511 / 0.63 / 0.673 / 0.674 / 0.764
#   真·待补 0.193 / 0.229 / 0.263 / 0.273 / 0.387
# 缺口落在 (0.387, 0.458)，取中点 0.42，两边各留 0.03 以上。
#
# **0.42 偏向「宁可说待补」**：真·已写里 0.385 那条（P4 N2B5）从此不再翻过来，
# 代价是用户可能把已经写过的再写一遍；反过来放过一条真·待补，用户会**漏掉不写**。
# 后者贵得多——这正是 P22 #1 那句「比 P4 那个反向错更糟」。
# 换算：旧口径 P4 方向的误标 2 条 / P22 方向的误翻 2 条；新口径 1 条 / **0 条**。
#
# 量过之后**决定不做**的一条：「把节拍自己的否定从句（否则 / 尚未 / 仍是…）剔掉再算」
# ——P22 #1 建议过，实测是**反向**的：剔掉之后分母掉得比分子快，
# 两条真·待补反而从 0.371 / 0.387 涨到 0.429 / 0.524（`p24/m1_variant2.py` 的 A 列）。按铁律退回。
# 同样退回的还有「把正文里『未定 / 缺位 / 没有写入』那种缺口自述小句也剔掉」：
# 它只把 0.387 压到 0.355，却把三条真·已写各砍掉 0.13–0.28（`m1_variant3.py` 的 B2 列）。
COVER_WINDOW = 3
COVER_MIN = 0.42
