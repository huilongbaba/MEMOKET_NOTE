...
| 11 | 侧栏宽 280 + 内边距 16（搜索框与树行对齐一条竖线） | ✅ 第 716 轮 |
| 12 | 图标描边宽度 | ✅ 第 716 轮（**负结果：两边实测都是 2px，lucide 默认值就对，不用调**） |
| 13 | 逐屏复核（浅色 + 深色 + 窄窗 1000×700） | ✅ 第 717 / 726–727 轮 |
| 14 | ⌘L 跳到 composer、键帽按 `.kbd` 收紧 | ✅ 第 724–725 轮 |
| 15 | 去掉左栏右边线（mac 分支本来就没有）、内容区顶部留白 | ✅ 第 721 轮 |
| 16 | UI_SPEC 改指向本文件 | ✅ 第 723 轮 |

### 明确不照搬的

- **顶栏（`.home-topbar` 52 高 + 右上角主动作胶囊）**。他们是**单文档应用**，
  顶栏那条就是全局动作条；我们有**标签栏**，那是桌面原生的结构。
  UI_SPEC §0 铁律：**风格迁移，布局不动**。硬把他们的顶栏套上来会同时有
  标签栏和动作条两条横带——正是第 714 轮刚减掉的那种堆叠。
- **`--ink-3` 取 `#8C8C8C` 而不是 `#999999`**（见 §1 的偏离说明）。
- **分组标题用 `--ink-2` 而不是 `--ink-3`**：分组标题是要读的（UI_SPEC §1.4）。
- **`--success` 不跟着别名成品牌紫。** 他们写的是
  `--success: var(--brand); --success-weak: var(--brand-weak)`——**在他们那边成立**，
  因为那个应用里没有任何别的绿。我们有 **diff 的「新增 / 删除」**：
  紫配红比绿配红差得多，而且紫在我们这边已经同时承担了选中、链接、品牌标记。
  绿留着（`--ins` / `.badge.ok`），红黄照旧。
