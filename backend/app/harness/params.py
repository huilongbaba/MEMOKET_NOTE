...
# 单次跑的成本上限，单位是**入 + 出的 token 总数**（计划 12.3 / [MECH] §7）。
#
# **在这之前完全没有上限**：`judge` 只说 continue / complete / blocked，
# 从不问「再花十几秒值不值」，而一次跑能花多少没有任何约束。
#
# 这个数不是拍的。批 23 把 `llm_usage` 按时间窗口贴回 `harness_rounds`
# 的 158 次跑（`scripts/run_cost_probe.py`，只读），一次跑的 token 分布是：
#
#     p25 20,192 · 中位 85,041 · p75 139,050 · p90 186,243 · p95 207,741
#     p99 225,561 · **最大 248,160**
#     按模式：note 中位 112,705 / 最大 248,160；eda 59,624；analysis 36,053；
#             四个 block 模式中位 11,439~14,505、最大 34,421
#     每轮：中位 26,483 · p90 46,690 · 最大 82,720
#
# 交叉验过一次：09-13~09-16 那段**真实用户使用**的 888 笔调用按 120s 间隔
# 聚成 47 团，中位 113,404——跟上面 note 模式的 112,705 几乎一样。
#
# 500,000 = 观测到的最大值的 **2.0 倍**，也高于「拿库里最长的一篇笔记
# （47,273 字符，是 4 月那份合成探针）线性外推」的 ≈383,000。也就是说：
# **在已经量到的 158 次跑上它一次都不会开火**，这是有意的——它是安全网，
# 不是控制器（同 `CONTINUE_MAX_TOKENS` 那条注释：一个会在正常使用里被撞到
# 的"安全网"就不是安全网）。它要拦的是工具循环空转、正文失控增长这类
# **形状不同**的跑。
#
# **超了是停下来告诉用户，不是静默截断**：停机原因 `cost_cap` 会进
# `harness_runs.stopped`、进 RUN_FINISHED、并单独发一条 `cost` 事件带上数字。
# 判断在**轮末**，所以最多会超出一轮的开销（上面那个 p90 46,690）——这是
# 有意的：轮中掐断等于把一轮的钱花了又不要它的产出。
#
# 0 或负数 = 关掉。
RUN_TOKEN_CAP = int(os.getenv("MEMOKET_RUN_TOKEN_CAP", "500000") or 0)
...
# 材料进 prompt 前的相关性筛（P8 问题 5）要不要**真的剔**。**P57 起默认开**（前八批默认关）。
#
# ## 它为什么关了那么久（P8b）
#
...
#
# ## P57 为什么开（**两条判据是开跑前写下来的，不是事后挑的**）
#
# P28 补齐三条前置（确定性回填顺序 / context 侧剥编号 / 剔到下限先再检索一次）之后，
# P56 在 P53 那 5 跑上重量了一遍，结论是「不够格」——差的不是判据，是 `refill` 的播种：
# 会触发下限的那几轮正好全部空手，于是下限一响就把刚判过的那批垃圾塞回来。
# P57 修掉播种（`hooks/note.refill_facts` 的两级台阶）和那 3 条误剔（月份归一），再重量：
#
# | P56 表那 7 个轮 | 给了 | 会剔 | 剩 | 撞下限 | `refill` | 最后进 prompt |
# |---|---:|---:|---:|---|---|---:|
# | `da080` r1 | 6 | **0**（P56 是 1，那条是误剔） | 6 | 否 | — | **6** |
# | `e78306` r1 | 6 | **1**（P56 是 2） | 5 | 否 | — | **5** |
# | `603dca` r4 | 9 | 7 | 2 | 是 | 两级都空手 | 兜底塞回 1 → 3 |
# | `603dca` r5 | 6 | 5 | 1 | 是 | 两级都空手 | 兜底塞回 2 → 3 |
# | `a941` r1 | 6 | 6 | 0 | 是 | 第①级 6 条 | **6** |
# | `a941` r3 | 6 | 6 | 0 | 是 | 第①级 6 条 | **6** |
# | `3a3a` r1 | 6 | 5 | 1 | 是 | **第②级 6 条**（P56 是空手） | **7**，兜底一条没塞 |
#
# · 判据①「7 个轮里没有一个第 1 轮空手」：5 篇的第 1 轮进 prompt 的是 6 / 5 / 6 / 6 / 7 条，
#   **没有一轮低于 `MIN_KEPT`**。P8b 那两个死法（da080 剩 2 条、3a3a 全剔弃答）都不再发生。
# · 判据②「误剔率不涨」：32 条剔除逐条读过（P56）是 29 对 / 3 误；P57 归一之后是
#   **30 条里 1 条误剔（3.3%）**，剔对的 29 条**一条都没被顺手救回来**。
#
# ## 真跑那 5 篇验过（`<scratch>/p57/{p57_run,cmp7}.py`，三列对照）
#
# 留下率（机读，用户原段逐字还在）**52/52 = 100% → 100%，没跌**；
# token **0.42M → 0.40M，没涨**（每轮 23,071 → 23,284）；
# 「无关材料」（带 EVT/PCBA… 的新段）**7 段 → 4 段**，而且 P53 #3 点名的
# `a941`（P55 是 5 段）和 `3a3a` 双双**归零**；引用编号查不到的 0 → 0；元话语 0；逐字重复 0。
# 「改用户原文」那一格尺子报 1，**逐条读过是尺子误报**：那条 `delete` 的锚点前 40 个字
# 跟用户原文撞了（原文「After moving to the reasoning or the text, Speaker B can go…」，
# 删掉的是 harness 自己上一轮写的「…, go through it again…」），留下率 52/52 是另一把独立的尺。
#
# ## 还没解决、开着也解决不了的两条（别把它当全好了）
#
# · `603dca` 那篇 `refill` 两级都够不着（8 种 query 组合全部空手，根因在
#   `kite_memory._cjk_terms` 挑 grep 词那一层，动它就是动 `recall` 全局），所以那两轮
#   仍然走 P8b 的兜底把刚剔掉的塞回来——塞回来的是 `Wi-Fi is not good.` 这类连线噪声。
# · `3a3a` r5 那 6 条华为芯片事实 `B_zero_overlap = 0`，**开着筛也剔不掉**。
#
# 开着时也剔不到 `relevance.MIN_KEPT` 条以下（剩下的按重合度留最相关的）。
# **不加档位**：加档位只是把「下限一响就塞回刚判过的垃圾」那条路留在原地、再多一个开关
# （P56 的原话）。要关就整条关：`MEMOKET_RELEVANCE_FILTER=0`。
RELEVANCE_FILTER = os.getenv("MEMOKET_RELEVANCE_FILTER", "1").lower() in ("1", "true", "yes")
