...
| **自动备份** | 后端每次启动一天一份笔记库备份（sqlite 在线备份，留最近 7 日 + 3 个月），帮助菜单「打开备份文件夹」；整库还能随时导出成 Markdown zip |
...
| **桌面版** | Electron 外壳（`desktop/`），后端打成不依赖系统 python 的可执行程序一起装进 .app。标签页、分栏可拖可折叠、前进后退、⌘F 页内查找、⌘K 跳转、⌘/ 快捷键表、外观跟随系统 / 浅 / 深 |
...
cd ../frontend && npm test                 # tsc + eslint + vitest + 11 个检查脚本
```
...
| `GET /api/notes/brief?q=` | 轻量笔记列表（不带全文）：⌘K 和 `[[` 补全每敲一个字用的；正文命中带片段和第一行正文 |
