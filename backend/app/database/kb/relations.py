...
#
# ⚠️ **这一段原来还有一句「库不到 333 个 unit 时这条判据**等于不启用**」——那句话是错的**
# （P81 量到、P82 量完判「不换算法」）。`MIN` 只是在小库上**换了个高度**：
# 有效门槛是 `max(6%, 20/total)`，193 unit 的库上 = **10.4%**，不是「不启用」。
# 六个库实测（P82，`KITE_DATA_DIR` 那份拷贝）：
#
...
#
# **兜底在够不着 20 的那一档（≤20 unit）是真兜住了，在 20–333 那一档没兜住**——
# 而真实用户正落在后一档：193 条记录的库比 2362 条的库**多砍掉一倍**的词。
#
# **P82 把「按注释说的那样真去不启用」在全库上量了一遍**（`recall_ruler --cf-common`）：
# 765 条里变了 **49 条，全部落在 `terrence-rewrite`（49/86 = 57.0%）**，其余五个库
# **逐格不动**（`MIN` 在 ≤11 unit 的库上本来就等于不启用，在 2362 unit 的库上本来就不绑定）。
# 掉 52 进 140、有召回 341 → 347。**49 条逐条读完：变好 16 / 变差 23 / 中性 10 → 退回。**
#
# **为什么会变差**（根因，不是猜的）：`common_term()` 在这个库上**兼着口水词兜底**。
# 关掉它之后冒出来当证据的是 `因此`(23.3%) `如果`(34.7%) `团队`(54.4%) `用户`(50.8%)
# `说明`(17.1%) `消费者`(11.4%) `连接`(13.5%)——**`_is_cn_filler` 那张固定表里一个都没有**，
# 今天挡住它们的只有 `common`。同样这几个串在 2362 unit 的真库上 df 本来就低
# （`因此` 0.3% · `团队` 4.2% · `说明` 2.4%），所以那边**谁都不挡也没事**。
# 于是关掉之后 i=39 / 295 / 296 / 686 / 692 整屏被 `因此` / `如果` 撞进来的噪声灌满。
#
# **所以这条判据换不了**（P82 判的）：P81 读出来的根因「单主题小库里主题词的 df 天然就高，
# df 高 ≠ 它不是证据」**成立**，但**同一个 df 判据在那个库上还兼着第二份工**；
# 只按库大小去关它，砍掉的是第二份工。要真修得先把「泛词」和「主题词」分开，
# 而 P29 已经量过实体层 / 主题层今天两头都不顶用——**那是另一条路，不是这个旋钮**。
COMMON_DF_MIN = 20
