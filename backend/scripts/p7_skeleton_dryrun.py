...
P4 实拍的 4 篇：e78306202d78 / a941efecd390 / df3b4f7e987d / 0eecee3d7b94。
"""
...
def half_sentence_beats(beats: list[str]) -> list[int]:
    return [i for i, b in enumerate(beats, 1) if len(b) == OLD_BEAT_MAX and b[-1] not in _ENDINGS]


def scan(db: Path) -> list[dict]:
    """只看 `user` 血缘的笔记（`corpus_lineage.load_notes`，只读）：demo / fixture / 脚本生成的笔记
    骨架是不是半句无所谓，不值得为它们打模型。"""
    kept, _dropped = corpus_lineage.load_notes(db, keep={corpus_lineage.ORIGIN_USER})
    out = []
    for r in kept:
...
async def regenerate(user: str, note_id: str) -> tuple[str, list[str]]:
    ...
    return spine, verify_beats(beats, content)
