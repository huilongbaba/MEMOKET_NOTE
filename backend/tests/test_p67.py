...
def test_伪相关反馈的种子_P67量完没动_P69读完那2颗才落的地():
    """P67 的原话：「候选 `scored[:2]` → `scored[:1]` 量完了、23 对逐条读完
    （`p67-lead1-15`，**掉错了（硬）0**）**但页边圆点 31 → 33**，那 2 段没读——**不改**。
    这条闸钉住那个 2，**改它就得先把上面那笔账重算一遍。**」

    **P69 ① 把那笔账重算了，也把那 2 段读了**（N1 的 L208 / L350，两段都是
    `unsupported` 的 `why` 从 `no_record` 翻成 `no_value`，账在
    `scripts/margin_dot_ruler.py` 的 `EXPECT` 上面 + 台账 P69 ①），于是这一刀落了地。
    这条闸**从「钉住 2」翻面成「钉住 1」**——再想动回去，同样得先把账重算一遍。"""
    src = _src(kb_search.rank)
    assert "for (_s, r) in scored[:1]:" in src, "P69 ① 落的地：种子是第一名，不是前两名"
    # **这道门逐字没动**：量的时候只动了取种子那一行（P61「换量程时阈值逐字保持原样」同族）
