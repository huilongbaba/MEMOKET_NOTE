...
def drop_already_written(content: str, text: str, *, threshold: float = 0.62) -> str:
    ...
    **教训：按聚合指标改机制之前，先把命中的原文抓出来看一眼。**
    ...
    for para in re.split(r"\n\s*\n", body):
        ...
        kept.append(para)
    whole = re.sub(r"\n{3,}", "\n\n", "\n\n".join(kept)).strip()
    # **句级那一遍（`drop_restated_sentences`）第 766 轮被摘掉了，不是忘了接。**
    #
    # 它本身的诊断是对的（段落级只能整段扔或整段留，而模型重写整节时新旧句子
    # 混在同一段里，difflib 被稀释到 0.4 以下），但接在这里有三处硬伤，
    # 审查时逐条实跑验过：
    #
    # ① **阈值的依据是假的。** 注释里「31 篇真实笔记，30.6% 的句子有孪生句」
    #    ——那 31 篇里有一篇 47k 字的合成性能探针（`shot-perf` 的「超长笔记」，
    #    100 个逐字相同、只有周数不同的小节）。排掉四个夹具用户之后，
    #    ≥0.9 的占比从 **30.03% 掉到 2.63%**；而且真实笔记上阈值**确实有影响**
    #    （0.55 → 0.70，命中 8.30% → 5.55%），不是「谷很宽、取哪儿都一样」。
    # ② **破坏 dedup 镜像协议。** `hooks/mirror._record_dropped` 只按段 / 行
    #    两级比较；句子从**一行内部**被剔掉时，这一段和那一行都不在 kept 里
    #    → 报**整段** → 客户端 `onDedup` 把整段从编辑器里抹掉，而服务端留着
    #    修剪后的版本。用户看到文字消失、到轮末才跳回来——正是 `mirror.py`
    #    自己 docstring 里说要消灭的那类漂移。
    # ③ **清单兄弟项只盖住一半。** `template_rows` 按前 8 个字分组、一组 ≥3
    #    才豁免，于是 `1. …` / `2. …` 这种编号清单**第 0 个字就分道**，
    #    永远凑不成一组；真实笔记里 538 条会参与判重的清单行只有 58% 拿得到
    #    保护，已经实际删掉过一条。
    #
    # **这是这一批里唯一会直接删掉用户看得见的文字的改动**，所以在三处都修好
    # 之前先断开：函数、单测、以及 `repeats.py` 那边只读不删的用法全部保留
    # （判据和修订候选那条线不动，它不删东西）。重接的前提写在
    # `docs/TRACELOG-harness.md` 批 4。
    return whole
