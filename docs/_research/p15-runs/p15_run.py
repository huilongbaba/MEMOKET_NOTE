"""P15 #1 真跑：done_criteria 挪到材料族之后、重复族之前 + citations_present 认 note:// 之后，同一份 harness 跑两篇：
...
async def run_one(note_id: str, intent_text: str, checked: tuple[str, ...], max_rounds: int, budget: float) -> dict:
    ...
    async for ev in loop.run(st, hooks):
        ...
        if et == EventType.STEP_STARTED.value:
            ...
            print(f"--- round {cur} end len={len(st.content)} fresh_units={len(fresh_units)} cited={r['fresh_units_cited']} "
