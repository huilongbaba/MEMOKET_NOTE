...
def truncated_citations(text: str, facts: list[str], exists) -> list[str]:
    ...
    **能查到就是真的**，形状好不好看不归这条管。
    ...
    ## 命名空间是**两个来源的并集**（P59 ③，回答 P58 ⑤）

    P58 走查 #4 看到壳上这条一个都不报，记下一个问题：「命名空间要不要算正文里
    已有的合法 id？」——**下面那行 `ns` 本来就算了**：`id_prefixes(valid)` 里的
    `valid = cited_ids(text)` 就是正文里的合法 id，跟材料那一份取并集。
    空只发生在**两边同时空**的时候。

    P59 ③ 量过（`<scratch>/p59/reach9.py`，p53/p55/p57/p59 四批 / 75 轮）：

    * 正文光靠自己就够得着的轮：**65/75**；一个合法 id 都没有的：10/75。
    * 材料那一半（逐轮材料清单是 P59 ⑤ 今天才记进 run json 的，所以**只有 p59 量得到**）：
      **20/20 轮都有带 id 的材料行**，一轮都没空过。
    * 「两边都空」（p59 这 20 轮）：**0**。
    * 这四批唯一一份残骸是 P53 `3a3a96354546` 的 `[terrence-8F6]`（活了 7 轮到终稿），
      **其中 3 轮光靠正文的命名空间就够得着**——判据今天这么写就抓得住它。

    所以 **P59 ③ 一个字没改**。P58 走查上那一格空，是**假模型**造出来的局面
    （不发工具调用 → `st.facts` 空）**加上**走查那篇壳笔记正文里一个合法 id 都没有，
    两个条件同时成立。真跑里没见过。
    钉「正文那一半自己就够用」的闸：`tests/test_p59.py::test_3_命名空间光靠正文那一半就够得着`。
    """
