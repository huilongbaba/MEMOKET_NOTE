 ...
 *
 *  跟「知识库里暂时没有找到相关内容」**不是一句话**：那句是「问过了，没有」，
 *  这句是「没问成，不知道」。压成一句就是替空气背书（跟 P46 #1 那条
 *  「`[]` 和 `null` 不许压成一个」同一个形状）。
 *
 *  而**光把上一段的答案留在屏幕上**比说错还糟：标签写着「按光标这段找的」，
 *  底下摆的是上一段的命中词和上一段的记忆卡——P78 在真壳上拍到过
 *  （一篇塞五段，第 2 段那一行跟第 1 段**逐字相同**）。 */
export const RECALL_FAILED_NOTE = '这一段没查成（没拿到回答）——先不摆上一段的记忆，改一个字或挪一下光标会再问一次。'
