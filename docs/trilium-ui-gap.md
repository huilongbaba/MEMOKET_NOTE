...
| 状态栏高度 | `min-height: 28px`，`font-size: .85em`，`padding-inline: .25em`，背景走 `--left-pane-background-color`，上边框 `--status-bar-border-color` — `$T/widgets/layout/StatusBar.css:4-14` | `height: 24px`, `font-size: 11px`, `padding: 0 10px`，背景走 `--launcher-bg` — `$M/shell.css:164-170` | **已做**：状态栏令牌化 | 高度 28px；背景改左栏色（状态栏在视觉上是左栏的延伸，不是启动栏的） |
