...
> **补充说明（导入实现后）**
> 本文档记录的是用 KITE **原生 API** 直接测出的基线。本项目的实现没有在交互
> 路径上使用原生 `recall()` —— 它改用零 LLM 的符号检索（~1 ms），并对中文加了
> n-gram 回退。所以下文的「查询 90–110 秒」和「中文召回弱」描述的是**为什么
> 需要那些改造**，不是当前系统的表现。详见 README 的「关键设计」一节。
