...
def test_2_门槛在两批量出来的缺口里():
    # P24 在 P4（26 条）+ P22（31 条）两批 57 条节拍上重量过（`p24/m1_variant3.py`）：
    # 剔掉正文里的占位格之后，**真·待补最高 0.387、真·已写里必须翻过来的最低 0.458**，
    # 门槛落在这个缺口里，两边各留 0.03 以上。
    # 旧口径（不剔占位格）没有缺口：真·已写最低 0.352 反而低于真·待补最高 0.387——
    # 那正是 P22 #1 两条真·待补被翻成「已写」的原因。
    assert 0.387 < sk.COVER_MIN < 0.458


def test_2_占位格不算写了_P22实拍两处():
    """P22 #1 实拍：表里的格子逐字写着「填写具体金额」，节拍说「把实际金额…填入表格」，
    旧口径把它翻成「已写（正文第 7 行起）」——用户读到这句就不会去填那张表。"""
    content = ("落地时要把各中介的方案放进同一张表里比较：\n\n"
               "| 中介 | 报价 | 判断依据 | 仍需核实 |\n"
               "|---|---|---|---|\n"
               "| A | 填写具体金额 | 填写原话或明确判断 | 填写待核实内容 |\n"
               "| B | 填写具体金额 | 填写原话或明确判断 | 填写待核实内容 |\n")
    beat = "待补：把本月各中介的实际金额、原话、买家反馈、时间范围和证据逐项填入表格。"
    assert sk.verify_beats([beat], content)[0].startswith("待补：")
    # 占位格抹掉之后行号和分段一个不动——翻过来的那句「正文第 N 行起」才指得准
    assert [ln for ln, _t in sk._paragraphs(sk._strip_placeholders(content))] == \
           [ln for ln, _t in sk._paragraphs(content)]
