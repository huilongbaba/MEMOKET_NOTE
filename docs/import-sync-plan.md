...
7. §4.3 用量记账（先估算，后钩子）；§4.5 并发。✅ 2026-09-13 记账部分：`util/llm.py` 每次调用把供应商响应里的 usage 记进 `llm_usage`（流式靠 `stream_options.include_usage` 的最后一帧），中间件按 X-User-Id / 路径标「谁、哪个功能」，`GET /api/settings/usage` 汇总今天 / 7 天 / 30 天 / 全部 + 按功能，设置页「模型用量」一节。KITE 抽取按块估一笔进同一张表（`kb/extract~`，1200 固定 + 字数 / 1.5 + 每条事实 60；钩子等上游）。§4.5 并发未做。
