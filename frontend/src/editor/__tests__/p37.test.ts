...
describe('P37 #5 待处置的改动层：关掉就当接受了，至少先说一句', () => {
  it('改动条上说了「关掉这个 app 就当接受了」', () => {
    // 量程：把那半句删掉，这条红。P37 量过 `note_revisions` / `harness_runs`
    // 重建不出那一层（见台账 P37 节），落库是下一批的事——在那之前不许它是静悄悄的。
    expect(appSrc).toContain('关掉这个 app 就当接受了')
  })

  it('分层面板那一段也说了，而且说清了「烧过的跑」那一段会留', () => {
    expect(layersSrc).toContain('关掉这个 app 就当接受了')
    expect(layersSrc).toContain('不会留到下次打开')
  })
})
