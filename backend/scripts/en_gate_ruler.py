...
EXPECT_QUORUM_CALLS = 375963     # `_strong_enough` 被问了多少次（P84：375864 → 375963）
EXPECT_QUORUM_TRUE = 1821        # 其中判 True 的（P84：1771 → 1821）
EXPECT_QUORUM_EDGE = 1531        # 判 True 且**证人正好两条**（少一条就翻。P84：1488 → 1531）
EXPECT_QUORUM_REJECT_AT_2 = 11   # 够了 quorum 却被后面那道门判回去的（只有这一档有）
# `--quorum` 那一行：**用户眼前**那 1347 条召回对，各自靠几条证人撑着
EXPECT_QUORUM_SHOWN = (1367, 301, 792, 1066)   # (召回对, 短查询·这道门不问, 正好两条, 这道门管着的)
