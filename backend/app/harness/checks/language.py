...
判据**宁可窄**（`junk_tail`）：三个条件同时成立才算——
  ① 位置：段落**最后**，前面是一个句末标点（空白可有可无，见 P55 #1）；
  ② 形状：2–12 个汉字，跟这一段其余部分 **2-gram 零重合**（是硬贴上去的，不是这段在说的事）；
  ③ 词表命中：`JUNK_WORDS`——**词表只从实拍来**（P19 一个 + P55 一个），不拍脑袋扩。
三条缺一条就不开火：只有 ①② 的是「模型写了个没说完的短句」，那是别的判据的事。
量程同 `no_foreign_script`：**只看这次跑新写的**；能自动修（把那几个字摘掉）。

## P55 #1：同一条判据上叠着两个洞，只补一个还是拦不住

P53 实拍：`3a3a96354546` 第 3 轮把「…应把这一步退回文案和设计重新处理。**做爰片**」
写进正文、一路留到终稿，而那一轮六维全打了分、`best` 还涨了一格——**没有任何一条判据看见它**。
拆开看是两件事：① 这个词不在 `JUNK_WORDS` 里；② 它前面**没有空白**，而 P19 那版的
`_TAIL` 要求句末标点后面必须有空白。只加词表，正则照样看不见它。两处一起改，缺一不可。

**词表加了什么、凭什么**：见 `JUNK_WORDS` 上面那段——12 批 47 份跑 / 229 轮扫了两遍
（形状扫 + 罕见字扫），全语料只有 `日本一本道` 和 `做爰片` 两个词，没有第三个，
所以只加 `做爰片` 一个。这条规矩（**词表只从实拍来**）是 P19 定的，P55 照着做。
...
#
# ## P55 #1 量出来的那一刀（12 批 47 份跑 / 229 轮 / `<scratch>/p55_junkscan*.py`）
#
# 两条口径各扫了一遍，**都没有第三个词**：
#   ① 形状扫（不带词表）：段末「句末标点 + 可有可无的空白 + 2–12 汉字 + 跟本段 2-gram
#      零重合」的候选，全语料只有 `日本一本道` 和 `做爰片` 两个；
#   ② 字扫：模型写出来、而「用户原文 + 这轮材料」里都没有的汉字共 765 个，按整份语料
#      出现次数升序逐个读，**只有 `爰` 一个是异常的**（全语料 1 次），其余 764 个
#      全是正常汉字（斗 / 削 / 势 / 训 / 唯…）。
# 计数：`一本道` 出现在 14 份跑里，**全部是开跑前正文里就有的**（`da080ca847cf` 的 `orig`
# 里 1 处），这 47 份跑里模型当轮流出 **0** 次；`做爰片` 当轮流出 **1** 次
# （p53 / `3a3a96354546` r3），一路留到终稿。
JUNK_WORDS = ("一本道", "做爰片")

# 段末：句末标点 + **可有可无的**空白 + 2–12 个汉字 + 段落结束。
#
# **空白原来是必须的**（P19 #5 那版），依据是实拍「…不再自动被视为进展。 日本一本道」
# ——中文句子之间不空格，句号后面空一格再挂四个字确实反常。**P55 #1 的实拍把这一半打掉了**：
# `3a3a96354546` r3 流出的是「…应把这一步退回文案和设计重新处理。做爰片」，
# **句号后面一个空格都没有**——两个洞叠在一起（词表里没有它、形状也认不出它），
# 只补词表那一个仍然拦不住。
# 空白改成可有可无，**另外两条一个字不动**：段末位置照旧、跟本段 2-gram 零重合照旧，
# 词表仍然是最硬的那一条——只满足 ①② 的正常短句（「…处理。明天再说」）一律不开火。
_TAIL = re.compile(r"[。！？!?…」』）)\]][ \t\u3000]*([\u4e00-\u9fff]{2,12})[\s。．.]*$")
...
def strip_junk_tails(text: str, tails: list[str]) -> str:
    """把那几条尾巴连同它前面那一个空白（有的话）摘掉，别的一个字不动。

    空白从 `+` 改成 `*`（P55 #1）：实拍 `做爰片` 前面没有空白，`+` 会一个字都摘不掉,
    于是 `fix` 跑完判据再看一眼还在——原子 fix 那一支会把整份改动丢回去，等于没修。
    """
    out = text or ""
    for t in tails:
