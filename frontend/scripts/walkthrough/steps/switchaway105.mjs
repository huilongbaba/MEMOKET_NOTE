...
//     这一步现造的那篇正是这一档 ⇒ 判 ⑤ **只判「点名的形状 + 理由还在」**，
//     **把实际那个篇名照实打出来**，判不了「点对了」。那一刀在 `docs/edge-cases.md` 上记着。
...
export default async function (d, [shotPrefixRaw, budgetRaw]) {
  const prefix = shotPrefixRaw || 'p105-switchaway'
  ...
  if (hit.length < 1) {
    ...
    // ⚠️ **「点了名」≠「点对了名」**（抬头「够不着」第 1 条）：照实打出来，**不判**。
    const right = hit.filter(([t]) => t.startsWith(`「${titleHead}`)).length
    console.log(`  （照实记·不判）点的名里对得上这一趟标题记号「${titleHead}」的有 ${right} 条；`
      + `读成「另一篇笔记」的有 ${hit.filter(([t]) => t.startsWith('「另一篇笔记」')).length} 条`
      + ' —— 那是 `notes` 闭包那一档，`docs/edge-cases.md` 上记着')
  }
  ...
  console.log('OK: 第 ⑫ 步五条判据全过（P95 串台 / P95 切回来 / P101 误伤 / P103 骨架 / P103 停机理由）')
}
