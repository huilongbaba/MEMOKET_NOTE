...
const PLAN = [
  ...
  },
  {
    ...
    // ⚠️ **⑥ 的「读库」这一格不在上面**，因为 `b2old.mjs` 自己那一行读的是
    // `localStorage.getItem('memoket.user')`——**整个前端里没有这个键**（真键是
    // `memoket-note-user`，`src/api.ts:86`）。真走查里 `|| 'terrence'` 那个默认值
    // 恰好等于身份，于是这条从来没露过馅；换成空库新用户就静默读了另一个人那一格，
    // 读回 `{"len":0}`。**「在真库上碰巧对」不是对**——这是「选不到 ≠ 没有」的
    // **默认参数**那一张脸。步骤脚本这一批不改（`PLAN` 之外一个字节不动），
    // 所以这一格**由闸自己按真身份问一遍后端**，这才是 P45 #1 / P44 #1 要的那条回归。
