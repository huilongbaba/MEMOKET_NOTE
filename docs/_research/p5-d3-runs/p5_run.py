"""P5 D3/D4: run 智能续写 (note mode) once on one real note, on the scratch copy.
...
S = Path("/private/tmp/claude-501/-Users-huilong-Skills-Bugfixing-Feishu/401a09f3-c80d-446c-a096-c81ed0fa949a/scratchpad/p5")
DATA = S / "p5data"
os.environ["KITE_DATA_DIR"] = str(DATA)
os.chdir(str(DATA))

WT = Path("/Users/huilong/Skills-Bugfixing-Feishu/MEMOKET_NOTE/.claude/worktrees/agent-ad08b44c886aa7181/backend")
...
async def run_one(note_id: str, budget: float) -> dict:
    ...
    mode = dataclasses.replace(
        ...
        max_rounds=max(1, min(20, MAX_ROUNDS_CAP)), review_each_round=False,
        rails_off=("save",))
    st = State(mode=mode,
