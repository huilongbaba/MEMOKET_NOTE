...
| **独立知识库工作区** | 知识库是最左侧的一级入口，与用户的笔记树分开。总览顶部可直达事实表 / 主题地图 / 时间线，所有知识页在同一个标签内切换并可返回；正文里的 `[引用]` 仍可悬停核对出处，笔记树仍显示每篇引用了几条（◆N）、摄入过没有（⇡） |
...
| **桌面版** | Electron 外壳（`desktop/`），后端打成不依赖系统 python 的可执行程序一起装进 .app。标签页（⌘W / ⇧⌘T 回原位 / ⌃Tab / 装不下时 ▾ 列表 / ⌘K 按名字找开着的标签 / 菜单栏「标签」）、分栏可拖可折叠、前进后退、⌘F 页内查找、⌘K 跳转、⌘/ 快捷键表、外观跟随系统 / 浅 / 深 |
...
| `GET /api/kb/tree` · `GET /api/kb/tree/children` | 知识库工作区内部的虚拟层级：分类层一次给全，进入一个分类时才取它名下的事实（实体超过 200 个、多段材料的各段也是按需取） |
