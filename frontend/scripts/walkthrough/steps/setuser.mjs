// 把这个窗口的身份设成老用户（`localStorage['memoket.user']`），然后刷新。
//
// **为什么要它**（P60 走查量具坑 #1）：P58 那份 `old/udd` 里已经躺着一份
// `memoket.user = terrence`（那个目录是上一批留下来的），这一批的 `old/udd` 是
...
export default async function (d, args) {
  ...
  console.log('设身份前:', await d.eval(`localStorage.getItem('memoket.user')`))
  await d.eval(`(localStorage.setItem('memoket.user', ${JSON.stringify(user)}), 'ok')`)
  await d.eval(`location.reload()`)
  await wait(6000)
  console.log('设身份后:', await d.eval(`localStorage.getItem('memoket.user')`))
