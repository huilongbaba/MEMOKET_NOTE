...
**后续（第 527–533 轮，2026-09-14）**：英文虚词和「speaker b」这类说话人标签不当查询词、不占 grep 槽，说话人实体不进符号通道；
同一个查询词出现不止一次每多一次 +1（最多 +2）；伪相关反馈——词面前两名挂的主题回头给其它候选 +1。
`backend/scripts/recall_selfcheck.py [user] [n] [seed]` 三个口径：seed 7 n 60 全文 60/60、前 40 字 60/60；
seed 11 n 200 是 197/195，「排除自己找同主题」73 → 98/200。掉了就是退化。
...
### 模型判的两条：成本不是翻倍，而且它抓到的是 prompt 的毛病

第 9 节第 5 步担心「成本翻倍」。实测**不是**：

| | 每场耗时 |
|---|---|
| 抽取一场会议 | 32–48s（读 30–57k 字原文） |
| 判一场会议 | **6–9s**（读 2–6k 字事实） |

判据读的是**抽出来的事实**，不是整场原文，所以它是抽取成本的约 20%。
「翻倍」那个估算假设的是逐 chunk 判——而整场会议一次抽之后，判的单位也
跟着变大了。全库 193 场约 25 分钟。

更重要的是它抓到了什么。五场会议、五个 `self_contained` 不达标、**五句
一模一样的判词**：相对时间和裸代词（「今年开始」「当前」「该项目」
「后来」）。这不是逐条噪声，是抽取规则里少了一条——逐场重试一次都修不好。

于是往 `EXTRACT_RULES` 里补了这条规则（写具体日期不写「下个月」，写具体
名字不写「这个方案」），同样三场会议重抽再判：

| 维度 | 改规则前 | 改规则后 |
|---|---|---|
| `self_contained` 均值 | 0.33 | **1.00** |
| `topic_fit` 均值 | 1.33 | **1.67** |
| 不达标场次 | 3 / 2 | 3 / 1 |

没到 2 分——模型还是会写「currently」「later」，剩下的那部分要它自己做
日期推算，是更难的一档。但**一条 prompt 规则把 0.33 抬到 1.00**，而这条
规则是判据指出来的。

这就是「判一批而不是判每一条」的意义：规则有毛病的表现是每一场都挨同一句
批评。端点是 `POST /api/kb/quality/judged?limit=N`——要花模型调用，所以是
POST，而且抽样不是全扫。

### 簇视图（第 9 节第 6 步）

主题地图默认看簇，点一个簇下钻到它里面的主题。`KnowledgeGraph` 一个字
没改——**一个簇就是一个没有父节点的主题**，`fact_count` 是它里面所有事实。
