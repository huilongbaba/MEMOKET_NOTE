...
class Verdict:
    ...
    # **判据自己动手那一轮，得对模型说一句**（P26 #3）。`fix` / `fix_done` 是「判据直接改
    # 正文」，而这两条路在 `middleware/checks.py` 里都是静默的：全修那一支直接 `continue`，
    # 连事件都不发；半修那一支只把 message 换成剩下没好的那条。于是**下一轮模型看到的正文里
    # 多了/少了自己没写过的东西，而没有任何一句话告诉过它**——它可能当成自己写的接着改，
    # 也可能原样再写一遍被删掉的那句。
    # P26 在 p13–p24（7 批 / 17 跑 / 77 轮）上逐条读出来的实例：`e78306202d78` 那一句
    # 「项目本身包含硬件、嵌入式和 APP，并按 KO、EVT、T0、DVT、PVT、MP 推进；」在 p22
    # （fix 还不存在）连着 r4/r5/r6 三轮原样活在正文里，同段的 `[terrence-1833-8F6]` 也贴了两次；
    # p24（fix 在了）同一篇终稿里两处都没了——**动手是真的，一句话也是真的没说。**
    # 这一行就是那句话：一句人话，说**做了什么**（不是哪里错了，那是 `message` 的事）。
    fix_note: str = ""
    # **这一条只是提个醒，不许把这一轮的打分短路掉**（P58 A）。
    #
    # 短路是这套 harness 里最贵的一个动作：那一轮的 `st.ev` 变成伪造的单维
    # `level=0`，于是**这一轮没有真分**——`complete` 到不了、排名进不了 `st.best`
    # 的正常比较、打分器的诊断也进不了下一轮的 prompt。它换来的是「一轮一个清楚的
    # 指令」，只有当那个指令**做得到**的时候才值这个价。
    #
    # P58 A 把 `citations_present` 的射程量死了（17 批 / 47 份跑 / 268 轮，
    # `<scratch>/p58/a_measure.py`）：判据自己报得出档的 **29 轮，29 轮全是
    # `located == 0`**——「这一轮写的字里没有一句能在材料里找到逐字的出处」。
    # 三条 `located > 0` 的分支（逐句点名 / 多义 / 第 2 轮点名）**一次都没开火过**。
    # 而 `located == 0` 那一档要的事（把这一段改写成材料里真有的那几条）是个
    # 改写任务，不是一条「补个编号」的机械指令：下一轮真照做的只有
    # **11/72 = 15%**（`<scratch>/p58/a_control.py`；p53+p55 单独看 3/9）。
    # 代价那一侧是确定的：**79/268 = 29% 的轮子因为这一条拿不到真分**
    # （p53+p55 是 11/38），比任何一条别的判据都多。
    #
    # 所以不是「让它闭嘴」（那等于删掉这条判据——29/29 都是这一档），
    # 是**让它说话但不收轮子**：事件照发、措辞照进 steer，打分器照跑。
    # 这条路 `middleware/checks.py` 本来就有（`STUCK_ROUNDS` 卡死放行 /
    # `JUDGE_FLOOR` 饿死放行），这里只是让判据自己也能要求走那一条。
    advisory: bool = False
