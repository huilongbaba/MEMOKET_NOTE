...
# ⚠️ **P84 抬了三个数**（568 → 583 · ("ai", 451) → ("ai", 466) · 收紧到 4 翻盘 213 → 211）：
# 那条「泛词 vs 主题词」的轴在 `terrence-rewrite`（193 unit）上把 63 个串从 `common`
# 手里捞了回来，于是那个库上多问了几趟这道门。**P77 ① 那条判（`ai` 一个词占了这道门
# 挡掉的绝大多数）逐格照旧**：466/583 = 79.9%（P77 量的是 451/568 = 79.4%）。
# **变的是分母不是形状**——`ai` 一个词还是占了近八成。
EXPECT_BLOCKED_OCC = 583
EXPECT_BLOCKED_KINDS = 14
EXPECT_TOP_BLOCKED = ("ai", 466)
EXPECT_FLIP_AT_2 = 11          # 放宽到 2（= 不设门）会翻盘的调用数
EXPECT_FLIP_QUERIES_AT_2 = 7
EXPECT_FLIP_AT_4 = 211         # 收紧到 4 会翻盘的调用数（P84：213 → 211，条数 100 没动）
...
#   ⚠️ **P84 之后这三个数各抬了一点**（375864 → 375963 · 1771 → 1821 · 1488 → 1531）：
#     那条「泛词 vs 主题词」的轴在小库上多放了 63 个串进证人名单，于是这道门多被问了几趟。
#     **P82 ② 那条判一个字没变**：1531/1821 = **84.1%**（P82 量的是 84.0%）——
#     **「判 True 的八成四押在第二条证人身上」这件事，换了分母还是这个形状。**
...
# **真正的脆在上面那个 84.0%（P84 之后 84.1%），不在这个 47%。**
# ⚠️ **P88 之后这三个数各退了一点**（375963 → 375959 · 1821 → 1816 · 1531 → 1526）：
#     新串的那条**主语面**轴（`kb/topic_face.WhoFace`）把 9 个串挡回了 `common`，
#     于是这道门少被问了几趟。**P82 ② 那条判还是一个字没变**：
#     1526/1816 = **84.0%**（P82 量的正是 84.0%，P84 那一档是 84.1%）——
#     **「判 True 的八成四押在第二条证人身上」这件事，分母来回动了三次还是这个形状。**
EXPECT_QUORUM_CALLS = 375959     # `_strong_enough` 被问了多少次（P84：375864 → 375963；P88：→ 375959）
EXPECT_QUORUM_TRUE = 1816        # 其中判 True 的（P84：1771 → 1821；P88：→ 1816）
EXPECT_QUORUM_EDGE = 1526        # 判 True 且**证人正好两条**（少一条就翻。P84：1488 → 1531；P88：→ 1526）
EXPECT_QUORUM_REJECT_AT_2 = 11   # 够了 quorum 却被后面那道门判回去的（只有这一档有）
# `--quorum` 那一行：**用户眼前**那 1347 条召回对，各自靠几条证人撑着
EXPECT_QUORUM_SHOWN = (1369, 302, 793, 1067)   # (召回对, 短查询·这道门不问, 正好两条, 这道门管着的)
# ⚠️ P84：(1347, 298, 778, 1049) → (1367, 301, 792, 1066)。**P82 ② 那条判照旧**：
# 792/1066 = **74.3%**（P82 量的是 778/1049 = 74.2%）——用户眼前那一屏还是四分之三
# 押在「正好两条证人」上。**换的是分母，不是形状。**
# ⚠️ P88：(1367, 301, 792, 1066) → (1369, 302, 793, 1067)。召回对 +2 是那条主语面轴
# 在 i=26 / i=293 各多摆出一条（i=293 那条正是「离开安克」的逐句出处）。
# 793/1067 = **74.3%**，**逐位相同**——分母来回动，形状纹丝不动。
