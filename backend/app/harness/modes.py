...
_FACTUAL_GROUNDING = Dimension(
    ...
    "数字但知识库里查无此事；把『有一次讨论』『会上大家』『我们测试发现』"
    "这类材料中不存在的具体事件写成真实发生过，也算编造，即使句中没有人名或数字。"
...
_PROSE_QUALITY = Dimension(
    "prose_quality",
    "判断文字能否作为成稿直接交给读者。若上下文里有【这次跑新写的句子】，"
    "只评价那一块；打磨模式没有该块时才评价整篇。"
    "2 分：表达自然、具体、句子完整，段落之间按真实的因果/转折/递进推进；"
    "标题说实际内容；没有写作计划标签、解释写作过程的旁白、证据审计口吻、"
    "套话、口号、抽象名词堆叠或明显病句，读起来像作者写完后认真改过。"
    "1 分：基本可读，但仍有一处明显的模板腔、平铺罗列、突兀跳转或别扭句子。"
    "0 分：主要内容像提纲扩写、材料汇报或通用 AI 文案，尚不能直接进入笔记。"
    "判为 0/1 时，note 必须逐字引用最需要修改的 1-2 个短句或标题，并说明如何改；"
    "不要只写‘不够自然’这类泛评。",
)
...
# "检查结构"实测没用。
# 分段写作缺的那一条：**这五个维度全在问「写得好不好」，没有一条在问「够不够」**。
# 第 605 轮真跑实拍：用户说「把硬件、APP、市场三条线**各写成一篇**」，三节各跑
# **一轮**就五个维度全 2 分判 complete，交出来 620 / 434 / 429 字——不是一篇，是一段
# 半。而 `material_used_up` 是 false，硬件那档知识库里有 412 条事实，这一节用上了
# 四条。短、干净、扣题、有引用、不重复的残篇是这个闭环的最优解，因为没人问它够不够。
#
# 单篇 harness 有 `beat_coverage` 管这件事（节拍是不是都落到正文里了），分段这边
# 没有 spine/beats，能对照的只有分段主题本身和总体目标——两样打分器都拿得到
# （routers/writing_plan.py 的 `_score_context`）。措辞照抄 `beat_coverage` 的分寸：
# **不要求写到极致**，只要求标题点到的每一面真的在正文里落了地。拿一个永远满足不了
# 的维度去打分，闭环就不收敛（`for_run` 的注释里记着代价）。
_SECTION_COVERAGE = Dimension(
    "section_coverage",
    "达标：分段主题点到的每一面都有对应的、具体的内容落在正文里——不要求"
    "写到极致，但读完能知道这件事具体是怎么回事；不足：只开了个头就收尾，"
    "标题点到的某一面完全没写，或者通篇只给了原则和口径，没有落到具体的"
    "项目、时间、数字或决定上。",
)
