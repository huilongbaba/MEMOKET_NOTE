...
class UserMemory:
    ...
    def common_term(self):
        ...
        **拆汉字闸那条路也量完了：不通**（收 P88 ③）。全库对拍变了 6 条、
        ...
        ## P92：**P90 自己点名的那条替代路也走完了，还是不接**（一个字节没改）

        P90 留的是「真要拆汉字闸，该走 `topics` 轴 + 英文专用低门槛（≈0.60），不是主语面」。
        这一批把它走完：留出集做厚到 **137 条**（六个库上 `topics` 面判得了的英文串**全部**），
        **门槛真的有缝**（产品那一堆 9 个串上：人标「不泛」.450–.663 / 「泛」.708–.880，
        假捞回 0.0% 能撑到 0.705），**轴也判对了**——可**产出还是变差**：
        全库对拍 **11 条变、进 37 掉 26**，逐条读完 **变好 0 / 中性 2 / 变差 9**
        （`--cf-gates` 的 `en060` 档，标注 `p92-en060-11`）。

        **卡的不是门槛也不是轴，是第三格：「不泛」≠「值得当证据」。**
        放行的两个串（`agent` / `memory`）命中的是这个库里**同一句产品定位话的六种说法**，
        八格的屏一进就是半屏（i=645 是七进七出，出去的七条全是 `Ask Memory` 的具体事实）。
        ⚠️ **字面去重看不见这一族**（3-gram 包含度只认出六条里的三条）。
        全文和三张表在 `kb/topic_face` 文件头第 ⑪ 格；
        「这道汉字闸一共挡过三次进攻」那张清单在 `_HAS_CJK` 上面。
        """
