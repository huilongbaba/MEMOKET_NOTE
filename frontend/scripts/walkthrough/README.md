...
`steps/toast97.mjs`（P97 加：**⑩ 那句 toast 用户到底看不看得见**。读三次——
⑩ 那个时刻的读法 / 从连上就盯着 / **把录音机装在页面脚本之前再 reload**，
第三次才答得了「它活着的那几百毫秒里屏幕上有没有它」。
**「量具够不着」和「用户也看不到」是两件事**，三个数分开记）、
`steps/rounds99.mjs`（P99 加：**轮次卡在三种情形下各丢什么**，一个形状读四次
——跑之前 / 跑着 / **跑着切走再切回** / 跑完 / **重开之后**。
三个档：`away`（切走 + reload）、`crash-mark`（打个记号交给外头 `kill -9`）、
`crash-read`（崩完重开还剩什么）。**它只读不判**，判在台账 §B；
「库里 `harness_rounds` 有几行」不在它射程里——那个在外头拿 sqlite 数）、
`steps/skel103.mjs`（P103 加：**`onSkeleton` 那处误伤的实拍**——两档（`stay` / `away`），
差别只在「点完『智能续写』切不切走」，判据是**库里那一篇的 `spine` 第几秒变非 0**。
⚠️ 它**必须跑 `LLM_MODE=adv`**：`--mode ok` 的假模型压根不回骨架 JSON，两档都读 0；
⚠️ **打完字 8 秒之内就得点**（前端还有一条后台骨架），而且**别拿「第 1 轮出现了」当骨架到过的凭据**
——`ROUND_START` 排在它前面。三条都逐字写在那份文件抬头，别删）、
`steps/toast103.mjs`（P103 加：**三条 toast 切走之后弹不弹**。
三条里只有 `onCost` 造得出来（`MEMOKET_RUN_TOKEN_CAP=1` 一设，第一轮末尾必发），
所以它拿 `onCost` 当那三条的实拍；形状那一半由 `frontend/scripts/check-harness-guard.mts` 第 ⑥ 条核。
**400 毫秒一轮地攒并集**——toast 活几秒就没，隔几秒读一次会整条漏掉）、
`steps/rounds101.mjs`（P101 加：**关掉重开那一屏右栏有几张卡**，
**改前 / 改后同一个探针跑两趟**（同一个壳，只换 `Resources/web`）。
...
| 占位符 | 换成什么 | 为什么不许写死 |
|---|---|---|
| `@LLM_PORT` | 这一趟真正的假模型端口 | 抄错端口这件事在结构上没地方发生（P68 那条坑） |
| `@LAST_NEW_NOTE` | **这一趟里最近一步现造出来的那篇**的 note id | **每一趟造出来的都是新的一篇**。P80 问题 #7：`old2.txt` 里写死着 `6cec5c7275c8`，那是**上一批**现造的，真库里没有 → `openNoteById` 当场 FAIL，看起来像产品坏了 |

`@LAST_NEW_NOTE` 的来源是**上一步自己打在日志里的那一行**（`steps/b1old.mjs` 的
「新建出来的 note id:」），不是猜、也不是回头查库——**查库会查到上一趟留下的那一篇**，
那正是这条坑的另一半。这一趟还没有哪一步造过新笔记就用它 → `go.sh` **当场出声并记一次失败**，
**绝不拿上一批的 id 顶上去**。

**「步骤脚本里不许再出现写死的 note id」有闸看着**：`check-walkthrough-runnable.mts` 第 ⑧ 条
——**摘掉整行注释之后**扫 `steps/*.mjs` 和 `go.sh` 的**代码行**，12 位十六进制当场红（例 / 反例各 4 条）。
**本文件和注释不在射程里**：上面那个 `6cec5c7275c8` 是这条坑的现场，写下来是为了让人认得出它，
把它也判红等于「记住这条教训」这件事本身犯规。
