...
def test_量程没收住的判据在stale那一档也会开火():
    """反过来那一半：一条不看「这次跑写没写」的判据，两档都开火。
    没有这条，上面那条测的可能只是「这条判据根本不开火」。"""
    b = cd.measure({"user": [_row("n1", _DUP_LISTS)], "script": [], "fixture": []}
                   )["checks"]["no_repeated_lists"]["by_origin"]["user"]
...
def test_这个入口只读():
    ...
    assert "db_guard.Watch()" in src, "跑批夹 Watch()，不靠自报"
