...
>
> **这份是证据基础，不是现行架构。** 它论证的重构已经做完了——现行架构看
> `docs/harness-framework.md`（第 23 节是落地记录）。留在 `_research/` 里是
> 因为下面那些实扫数据（54 个模块的 import 图、三条 harness 的步骤矩阵、
> 23 个 SSE 事件名）是当初那个判断的依据，值得能被翻回来。
