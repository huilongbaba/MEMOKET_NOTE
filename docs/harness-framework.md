...
backend/app/
  harness/                 agent 运行。不认识 FastAPI，不认识 sqlite
    ...
    checks/                  25 条代码判据 + rubric.py（模型打分）+ pick.py（打翻哪一维）
      ...
      · relevance（P8 问题 5：材料进 prompt 前的零模型相关性筛——只剔「从 >1000 条的主题里抽样回来
