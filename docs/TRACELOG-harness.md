...
| 维度 · probe | 批 9（半份上下文） | 批 10（修好之后） | 变化 |
|---|---|---|---|
| `fits_context` · eda/heading_flood | 1.22→0.78 掉 0.44，p=**0.105**，不显著 | 1.07→0.53 掉 0.53，p=**0.0015** | **显著了** |
...
该动判词了，接线已经不是瓶颈」。**接线还真是瓶颈**——把它少拿的那半份上下文
补上，同一条 probe 从 p=0.105 变成 p=0.0015。**没动一个字的判词。**
...
批 9 ③ 那条「退步」的担心没有复现）；`right_kind` 从「只动了一点」变成
...
4. **`kb_conflicts` 那 4 行里至少 3 行是误报**，跟 `conflict_confirm.py` 记的
   实拍完全一致。这是「未裁决的不许当成取代」这条设计的实证依据，不是推测。
