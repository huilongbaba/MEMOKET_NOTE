...
REQUIRED_MW = [
    ...
    # 批 15：`compact` 退休，换成 `sections`（小节索引 + 当前小节逐字 +
    # read_section，计划 3.1 / [CE] §7）。要的能力没变——「续写 prompt 不能把
    # 整篇原样塞进去」——变的是**用什么换**：摘要是有损的替换，索引是无损的指针。
    ("sections", "续写 prompt 的正文：小节索引 + 当前小节逐字"),
