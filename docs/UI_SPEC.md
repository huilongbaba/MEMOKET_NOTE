...
**`ink-2` 的浅色值是 `#6E7682` 不是 APP 的 `#86909C`**：源仓库自己注明了
`#86909C` 在白底只有 3.24:1、过不了 WCAG AA，已经调成同色相的 `#6E7682`。
这条要照抄**调整后**的值，不要回头去抄 APP 原值。
...
| 主按钮 | 底 `--brand`、白字、`box-shadow: 0 1px 2px rgba(20,16,40,.14), 0 5px 14px -6px rgba(74,31,156,.30)` | `:225` |
...
| | 值 | 源 |
|---|---|---|
| 高度 / 圆角 | `30px` / `16px`（胶囊） | `design-detail.css:281` |
| 默认 | 底 `--mk-surface`、边 `--mk-line`、字 `--ink-2`、12.5px/600 | 同 |
| hover | 字 `--ink`、边 `--ink-3` | `:282` |
| 选中 | 底+边 `--brand`、白字 | `:283` |
...
4. **对比度**：主按钮白字对 `#620FF0` 是 6.1:1（过 AA）；`--ink-2` 用的是源仓库
   已调过的 `#6E7682`（4.6:1）。深色那一套照抄源仓库（它已经按 AA 调过）。
