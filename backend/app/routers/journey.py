...
def delete_day(date: str, user: str = Depends(current_user)) -> JourneyRunOut:
    ...
    if fails:
        ...
        # **路径的公共前缀只说一次**（P56 #4）。P52 走查实拍的那条 toast 是 5 行
        # 绝对路径、每行 100+ 字符，五行里前 90 个字符逐字相同——摊开的是前缀，
        # 不是信息。收法**不是**「详情看日志」：P21 立的规矩是「删不掉要吵」，
        # 把清单挪进日志等于退回那次修的问题（用户按的是「删掉」，他得知道剩下什么）。
        # 所以一条都不少，只是把目录提到抬头，每行只留目录里的相对路径 + 原因。
        # 目录自己那一行没有结尾的 `/`，光换 `f"{d}/"` 换不掉它——**它会是整段里
        # 唯一还摊着绝对路径的一行**（写这条闸时当场被它抓到的）。两条一起换。
        lines = [f.replace(f"{d}/", "").replace(f"{d}：", "这一天的目录本身：")
                 for f in fails[:10]]
        more = f"（还有 {len(fails) - 10} 个）" if len(fails) > 10 else ""
        raise HTTPException(
            500, f"这一天没删干净，{d} 下面这 {len(fails)} 个删不掉{more}：\n" + "\n".join(lines))
    mem = UserMemory(user)
    gone = 0
    for s in segs:
        if s.get("session"):
