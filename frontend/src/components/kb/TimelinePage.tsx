...
export default function TimelinePage({ actions }: { actions: KbActions }) {
  ...
  useEffect(() => { kbTimeline().then((m) => { setMonths(m); setOpenMonth(m[m.length - 1]?.month ?? null) }).catch(() => setMonths([])) }, [])
  useEffect(() => {
  ...
  const max = Math.max(1, ...(months ?? []).map((m) => m.facts))
      ...
      {years.map((y) => (
        <div key={y.year} id={'y' + y.year} className="tl-year">
          ...
          {y.months.map((m) => {
            ...
            const dmax = Math.max(1, ...m.days.map((d) => d.facts))
            return (
              <div key={m.month} className={'tl-month' + (open ? ' open' : '')}>
                <div className="tl-row" onClick={() => setOpenMonth(open ? null : m.month)}>
                  ...
                  <span className="tl-bar"><span style={{ width: `${(m.facts / max) * 100}%` }} /></span>
                    ...
                    {m.days.map((d) => (
                      <div key={d.date}>
                        <div className={'tl-row day' + (openDay === d.date ? ' open' : '')} onClick={() => setOpenDay(openDay === d.date ? null : d.date)}>
                          <span className="tl-label">{d.date.length >= 10 ? d.date.slice(8) + ' 日' : '日期不详'}</span>
                          <span className="tl-bar"><span style={{ width: `${(d.facts / dmax) * 100}%` }} /></span>
