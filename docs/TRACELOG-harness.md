...
## 批 26 · 整张 §5 总验收表重量（2026-09-18）

**这一批一次模型调用都没发，一行产品代码都没改。** 产出是一份**数**：
`docs/harness-upgrade-plan.md` §5 那张表的 16 行，每一行今天的真实值 + 和目标的差距。

按铁律第 1 条「先能看见，再动手」——那一栏的数大部分是批 5 / 10 / 13 / 15 / 16 / 18
量的，中间隔了十批（19→25）加了账本、checklist、decompose-then-verify、量程收紧、
`_parse_call` 修复、`st.steer` 去重，**没有人回头量过这张表**。

**复用而不是新写**：16 行里 14 行是靠仓里现成的评测台量出来的
（`dimension_sensitivity_bench.py --report` / `--coverage`、`claim_misfire_probe.py`、
`criteria_drift.py --no-save`、`run_cost_probe.py`、`corpus_lineage.py`、`db_guard`），
新写的只有两段一次性只读脚本（`harness_runs` / `harness_rounds` / `llm_usage` 的分布查询、
批 16 那套「自源头档」的重搭），都在 scratchpad，**没有一个进仓**。
[SITE] 那次「八份调研一次没提到仓里早有评测台」的教训这一批没有重犯。

---

### 整张表的新旧对照（**一行都不许漏**）

| # | 判据 | 旧「现在」（哪一批量的） | 批 26 实测 | 判定 |
|---|---|---|---|---|
| 1 | 段内重复字占比 | 最差 **42.9%**（批 2） | `user` 18 篇：最差 **36.4%**、中位 0.0%、>10% **1 篇**、17 篇精确 0.0%；`script` 最差 43.5%；`fixture` 全 0.0% | **口径变了**（42.9% 那篇是 `script`） |
| 2 | 第 N+1 轮最弱维度「持平」 | **65%**（[SITE]，200 轮） | **55%**（66 对，`harness_rounds`）；变好 44%、变差 2% | **口径变了**（换数据源 + 换维度集合 + 剔掉 229 对短路轮） |
| 3 | 反复跑同一篇单调下滑 | **3/6**（[EFF]） | **`user` 0/7**、`script` 3/7、探针 1/1 | **口径变了 + 变好** |
| 4 | 收尾仍 `continue` | **54%**（[EFF]） | `stopped` 非空 69 次拆成 **7 种**；`status='continue'` 的 18 次**全部带具名原因** | **变好（超额达标）** |
| 5 | 一次跑内重复查询占比 | 32.5% → **3.5%**（批 13） | `note` **8.6%**（最近窗口 1.6%）；block 模式 **14–40%**；`custom` **分母 0** | **变好，但拆出一个没接线的缺口** |
| 6 | judge 缓存命中率 | **0.0%**（批 15/16） | **2.24%**，而那 5 次是**整条请求逐字重发**，不是前缀命中 | **量到了；目标该撤** |
| 7 | `data_grounding` 等三条的误伤 | 24 篇上 **0**（批 16） | `user` **0/18**、`script` 0/19、`fixture` 0/433；反向臂抓住 8/8、1/1、1/1 | **不变（目标成立）** |
| 8 | `factual_grounding` 判法 / 候选原子率 | 干净 0/27、植入 18/27；真跑 5 轮 **0 候选**（批 18） | 探针逐字复现；候选原子率 `user` **0.89 个/篇**（7/18 篇 ≥1）；34 份续写 **0 候选**（独立复现）；**真跑那一档量不了** | **补了一半，另一半量不了** |
| 9 | 「这一节材料够不够写」的照做率 | 待记（批 18） | **量不了，分母 0**（299 个相邻轮对里能判定的是 0 对） | **量不了** |
| 10 | `factual_grounding` 干净笔记判 0 | **19.8%**（364 格 72 格，批 7/18） | **48.1%**（156 格 75 格），**但是双峰的**：2 篇 94–100%、3 篇精确 0.0% | **口径变了，而且今天这个口径是坏的** |
| 11 | 没有位置级探测器的内在质量维度 | **2**（[MECH]） | 字面仍 **2**（`coherence` + `topic_fidelity`）；按批 19 的 `STOP_ONLY` 新口径是 **1** | **口径变了** |
| 12 | `material_use` / `fits_context` 满分率 | 24/25、4/4（[EVAL]） | `material_use` **156/156 = 100%**（更差）；`fits_context` **28/84 = 33.3%**（达标） | **一半变好、一半变差** |
| 13 | `material_use` 对「材料全剔光」 | 2.0 → **1.92**（批 5） | 2.0 → **1.67**（掉 0.33，p=0.5126） | **变好四倍，仍未达标** |
| 14 | `right_kind` 对「不存在的图片引用」 | **0.11 → 2.0（反的）**（批 5） | **2.0 → 0.0**（掉满档，p=0.1025） | **方向已修正（达标）** |
| 15 | `data_grounding` 判法 | 代码判 ✅（批 16） | 代码判**先跑、命中即短路**；不开火的轮次仍由模型兜底 | **量到了，措辞要加限定** |
| 16 | 共用一份 vs 这一条指令专属 | `prompt`/`custom` 两个（批 17） | 8 个里 **2** 个；上限 `MAX_ITEMS=4` + 代码判 **7 类**（逐类点名） | **不变（批 17 之后没再扩）** |

**变好 6 行**（3 / 4 / 5 / 12 的一半 / 13 / 14）、**变差 1 行**（12 的 `material_use`）、
**口径变了 5 行**（1 / 2 / 3 / 10 / 11）、**量不了 1 行半**（9，和 8 的真跑那一档）、
**不变 3 行**（7 / 15 / 16）。

---

### 逐条读了什么（凡是要当结论的比例，都读过原文）

1. **段内重复，读了 3 篇。** `06647b9c2031`（`user`，36.4%）5 对命中全读完：
   「下一轮 10 台到货为 6 月 15 日…」逐字两遍、「EVT 准备走 KLR 包装…」逐字两遍，
   最后一对**从词中间接上**（「准备走 KLR」/「走 KLR」，sim 0.96）——跟批 25 读出的
   机器损伤同形。`user` 第 2/3 差的两篇 ratio 精确 0.0%、`find_restated` 一对都不返回，
   **没有片段可读**（这正是批 2 说的「中间没有连续带」）。对照 `1da5a3c9767b`
   （`script`，43.5%）读出来是另一种形状——整段换两个字重说一遍
   （「官网**应**优先展示」vs「官网**也应**优先展示」），soak 多轮续写的产物，
   **不能拿来算 `user` 的比例**。
2. **judge 缓存，读了全部 6 行 `cached>0` 的记录**（含 bench 那一支）。命中值全是
   3246/3249、1607/1610、1597/1600 这种 **≈99.8% 全命中**——它们不是「部分前缀命中」，
   是**整条请求逐字重发**。135 次 `feature='judge'` 调用里 5 次 = 3.7%。
   **批 16 的结论没有被 2.24% 推翻**，反而被这 6 行坐实了。
3. **`factual_grounding` 判 0 的那两篇，读了材料块原文。** `309f19202309` 的 12 条
   `derived_facts` 里 3 条是结构残骸：F1 是**图片 alt 文本**
   `![为一篇中文公司汇报笔记制作一张专业、克制、现代的概念插图…](/api/assets/…)`、
   F3 是 **markdown 表头行** `| Method | Token | SS-User | SS-Asst |…`、F10 是整块列表。
   `06647b9c2031` 同样 3/12（三条全是整块列表，第一条吞了「范围边界：」底下四行）。
   而 `3a3a96354546` 的 `derived_facts` 是 **0 条**——材料块为空，判据按
   「没给材料就算达标」处理，判 0 率 0.0%。**三篇读下来，48.1% 量的是
   「bench 在 markdown 重的笔记上摘不出像样的材料」，不是判据的毛病。**
4. **`data_grounding` 三条的 0 误伤，没有片段可读（开火 0），所以读的是反向臂。**
   把正文里一个真统计量改成 883.45（源头保留原文）→ 抓住 **8/8**；
   `309f19202309` 表里 `67.14 → 883.45` → 抓住 **1/1**；砍掉表里一格 → 抓住 **1/1**。
...
1. **`corpus_lineage` 判的是「笔记」，不是「跑」——`harness_runs` / `harness_rounds`
   的血缘它判不了。** 我按 `key` 里的 note_id 分了一次血缘，`user` 那一桶看着很干净
   （单调下滑 0/7、持平 51%），**但那是假的**：探针脚本刻意拿真实 `note_id` 当种子
   （`block_rounds_probe.py` / `revision_ledger_probe.py` 的 docstring 逐字写着
   「拿假笔记跑出来的分布不作数」），于是一次脚本触发的跑会被判成 `user`。
   `run_cost_probe.py` 的 docstring 早就写过同一句「绝大多数是测量脚本触发的，
   只看形状不算生产发生率」——**这是批 6 那条规矩的新形态**：
   *一个分类器的输入类型，和你要分类的那个东西的类型，可能根本不是一个东西。*
   写进了 `harness-framework.md` §21 和 §5 表头的口径三条。
2. **计划 2.4「账本摘要进 prompt」只接了 8 个模式里的 1 个。**
   `gap_summary` 全仓唯一的读者是 `hooks/note.py:188`——`section` 和六个 block 模式
   一个都没接；2.5（`agent_loop.BARREN_STOP`）它们倒是有。
   而批 13 的隔离实验量过「只开 2.5 = 17.2%、2.4+2.5 才 3.5%」，
   **今天实测 block 模式 14–40%、note 8.6%，正落在那一档上**。
   这不是推理出来的，是先看见数再去代码里找到的。
3. **批 22 修的 `History` 丢 `max_rounds` 那类 bug，今天在数据上兑现了。**
   批 22 记「158 次跑 17 次跑满，而库里 `stopped='max_rounds'` 是 0 行」，
   今天 **5 行**，时间全在 2026-09-18T09:56–10:03（批 22 之后）。
   *一个修复有没有生效，最硬的证据是那张表里第一次长出该有的行。*
4. **`custom` 模式的 `tool_calls` 合计是 0——那一行的重复查询率分母为 0，量不了。**
   照批 22/23/24 的规矩据实写「量不了」，没拿这个 0 当成绩。
5. **`right_kind` 那一行「不显著」是数学上的，不是判据的问题。**
   掉分 2.0（满档），p = 0.1025——n=1 篇 × 3 次时排列总数只有 C(6,3)=20，
   **两侧 p 最小就是 0.10**。这份语料里能取到 `chart-block` 的只有这一篇。
   *把「不显著」读成「判据不灵」，会去改一条其实已经修好的判据。*

---

### 下一批该打哪几行（排序 + 理由）

**排序依据是「能不能改 × 改了值不值」，不是分数高低。**

1. **第 10 行的口径（`derived_facts` 摘不出像样的材料）。**
   **最值钱，而且是纯代码改动、零调用可验。** 它今天污染的不止一行：
   `material_use` / `factual_grounding` / `numbers_from_tools` / `data_grounding`
   四维在 bench 里全靠这一份材料，而 bench 的报告自己写着这四维是「上界」——
   **上界建立在一份含图片 alt 文本和表头行的材料上，那个上界是假的**。
   修法窄：`derived_facts` 切句之前先剔掉 markdown 结构（图片 / 表格行 / 围栏 / 列表块）。
   验法现成：改完 `--report` 重跑一次，`309f19202309` 和 `06647b9c2031` 那 78 格
   会自动重跑（`cell_key` 带正文指纹），**两篇的 100% / 94.1% 会不会掉下来就是答案**。
   *在一份摘坏了的材料上量出来的灵敏度，比不量更糟——它会让人去改判据。*
2. **第 5 行的缺口：把 2.4 接到 `section` 和六个 block 模式上。**
   **第二值钱，因为它是唯一一条「数和代码已经对上了」的。** 批 13 量过 2.4 的效果
   （32.5% → 3.5%），今天又量到没接它的那六个模式正好停在 17.2% 那一档（14–40%）。
   不是猜。注意 `custom` 的分母是 0，接完也验不了它——**别把它算进验收**。
3. **第 8 / 第 9 行缺的三列探针**（`claim_atoms` / `fired_checks` / `abstained`）。
   **改动极小（三列整数 / 一个 JSON 数组，不落用户正文），但它解锁两整行。**
   而且 9.2 / 9.4 从批 23 起就在等 `harness_edits` / `depth_dropped` 长出行来——
   **这批再确认一次：`harness_edits` 仍然 0 行、`depth_dropped` 仍然 0 行、
   `harness_snapshots` 0 行。** 光等是等不来的，得先有跑。
4. **第 13 行 `material_use`。** 它是这张表里**唯一一条变差的**（96% → 100% 满分），
   而且掉分 0.33 / p=0.5126 说明判据基本没牙。但**排在第 4 是因为它可能是第 1 条的
   下游**——材料摘坏了，「材料全剔光」和「材料没剔光」在打分器眼里本来就差不多。
   *先修上游，再判断下游还剩多少问题。*
5. **第 6 行：把目标从「> 0.7」改掉。** 不是改代码，是改这张表。
   今天已经三条依据说死了追加式不可行（正文 38.8% 的轮对在变短、
   追加式要把上一轮的分数留给打分器看、judge 只占全库 token 的 1.6%）。
   **留着一个已经证明达不到的目标，下一个人会去追它。**
6. **第 14 行补样本量**（排最后）。方向已经对了，缺的只是能取到 `chart-block` 的第二篇。
   **这一条要花真跑的钱，而它换回来的只是一个 p 值。**

**明确不建议现在做的**：第 11 行「给 `topic_fidelity` 补位置级探测器」。
`coherence` 那一条走过同一条路——批 19 先用 `scripts/coherence_denominator.py`
量分母，量完的结论是「不该做成判据」（148 个相邻标题对跳级 0、42 个兄弟组只有 1 组
全带编号）。**`topic_fidelity` 的分母一次都没量过，先量再说。**

---

### 闸

后端 **1888 passed**（一条没动）、前端 **52 个文件 263 条**（一条没动）。
**这一批没有改任何产品代码，所以没有突变验**——突变验是给新判据的，
这一批交的是数，它的「突变验」是第 2 节那四条逐条读原文：
**每一个当结论用的比例，都有一段能对着读的原文，或者一句「分母 0，量不了」。**

`harness-framework.md`：§21 补一行「血缘判的是笔记，不是跑」。
**Mode / 工具 / check / middleware 四个数一个没动**，`test_doc_counts` /
`test_directory_map` / `test_dimension_method_gates` 照绿。
...
碰真库的每一处都只读、夹在 `db_guard.Watch()` 里、**命令不经过任何管道**、
单独打 `EXIT=$?`。**没有读也没有写 `~/Library/Application Support` 那一份**（62 篇那个库）。
`.local/criteria_drift/latest.json` **没有被覆盖**（`criteria_drift.py` 一律带 `--no-save`，
mtime 仍是批 25 收尾的 18:45），也没有用 `--db` 换过库。

### 这一批的实际成本

**模型调用 0 次、0 token。** 16 行全部量在已有的数据上：
`harness_rounds` 462 轮 / `harness_runs` 108 次 / `llm_usage` 5283 行 /
`dimension_sensitivity_log.jsonl` 1785 行（有效 510）/ `harness_stress_log.jsonl` 200 轮 /
482 篇笔记的确定性判据重算。

**顺手把真跑的价签也更新了**（`run_cost_probe.py`，批 24 的数是 85k / 113k）：
一次跑中位 **87,410** token、p90 180,561、最大 248,160；**`note` 模式中位 116,503**；
每轮中位 26,892。真实用户使用那几天按 120s 聚类的上界是中位 113,404、最大 610,198。
**第 1 条建议（修 `derived_facts`）之所以排第一，一半理由就是它零调用可验。**

---

## 批 27 · 批 26 排的前三条（2026-09-18）

开工 HEAD `4bc59bc`。**这一批一次模型调用都没发**，改的全是**口径和探针**：
A 修 bench 的材料切句（§5 第 10 行）、B 把计划 2.4 接到剩下的七个模式（第 5 行）、
C 补三列探针解锁第 8 / 9 行。验收靠 **22 条突变验** + 全量闸。

**一句话结论**：三条都落地了，**但三条的「新数」都要等一次真跑**——
A 的 78 格作废重排了（`--report` 证明得了旧数作废，证明不了新数），
B 和 C 落的是接线和列。**这一批没有一个数是拿接线冒充的。**

---

### A —— `derived_facts` 的切句口径：**两条独立的根因，不是一条**

批 26 定位的是切句。修的时候发现**它只解释了两篇里的一篇**。

#### ① 切句（两篇都中）

老口径是把**整篇当一个串**按 `[。！？]` 切。markdown 重的段落里一个句号都没有，
于是一块表 / 一整块列表 / 一行图片语法**整块变成一条「事实」**。

改成**先按行剔排版、再逐行切句**。逐行切顺带钉住另一件事：
一条「事实」再也不会横跨两个结构块（老的 `06647b9c2031` F1 一条吞了
「范围边界：」底下四行）。

**第一版是错的，而且是一条批 8 留下的老闸抓住的。** 第一版把表格行和围栏内容
**整条剔掉**——干净利落，两篇 markdown 重的笔记上立竿见影，
`test_真正发出去的那一格带的就是生产那份上下文` 当场变红：
**`chart-block` 那几条 probe 的选区本身就是一整块图**，剔干净之后材料是空的，
而 `data_grounding` / `numbers_from_tools` 正是靠材料里那几个数判的。
*一个可能为空的量程，就不是量程。*
...
| 档 | 是什么 | 怎么处理 |
|---|---|---|
| 真剔掉 | 标题行、分隔线、表格的 `\|---\|` 行、围栏线本身、行内 `![…](…)` | 里面没有事实 |
| 第一档 | 普通行；去掉 `-` / `>` 标记之后的列表行、引用行 | 散文，优先 |
| 第二档 | 表格数据行、围栏里的内容 | 只能当语法读，散文摘不满 12 条时才拿它凑 |

**围栏那一处本来要写状态机，量完发现不需要。** `06647b9c2031` 正文里有 **3 条**
围栏线——奇数，开头那条的「开」没了（这篇正是 §5 第 1 行那篇段内重复 36.4% 的
机器损伤笔记）。纯 toggle 会把整块 mermaid 判成「围栏外」，修完切句之后
F10–F12 三条「事实」是三行 `A[…] --> B[…]`。第一版补了一条
「带语言标记的一定是开」的规则去猜；**第二版把围栏线本身剔掉、围栏内容排第二档，
奇数偶数都一样，那条规则整个不需要**。*不需要状态机的地方不要状态机。*

**顺手修的第三件事：同一句话只占一个格子。** `06647b9c2031` 的
「下一轮 10 台到货为 6 月 15 日…」在正文里逐字重复了八遍，
于是 12 条材料里 **8 条是它**——材料块看着满，真正不同的事实只有两条。
重复是**从词中间接上**的（批 25 读出的机器损伤形状），所以判的不是逐字相等，
是「一条是不是另一条的片段」。**这不是切句的毛病，两件事分开记。**

#### ② 量程（**只有一篇中，批 26 提醒的那个 13% 成立**）
...
```
309f19202309  正文 30588 字 · 打分器只拿到尾部 4546 字（14.9%）
              12 条材料全部落在前 21%
              **12 条里只有 1 条**的原句在打分器手上那段正文里找得到
```

**打分器判 `factual_grounding` 0 是它判对了，坏的是这份材料。**
另外四篇正文都短于 `keep_last`（整篇逐字给），所以这条只中一篇——
它跟切句是两条**独立**的根因，而批 26 的 48.1% 是两条叠在一起的结果。

修法：材料只在**打分器真的看得见的那段正文**里摘。
`whole-piece` 那一档递的是整篇逐字，它的量程也是整篇——两档各自自洽。
中间还有一个坑：`body_for_scoring` 的产出前半截是目录行
`- 第 N 节 · 标题 —— 第一句（N 字）`，那是**排版**，摘进材料等于把三条结构残骸
换个形态放回去（`test_材料里不许出现目录行本身` 抓住的）。分界那句话抽成了
`score_context.VERBATIM_MARK`，**脚本不许另抄一份**（跟「更正行必须由生产那个
函数写」同一条纪律，另有一条词法闸钉着）。

#### 修完的确定性对照（**零调用**，`db_guard.Watch()` 全程夹着）

| 笔记 | 结构残骸 | 同句重复 | 材料原句在打分器看得见的那段里 |
|---|---|---|---|
| `309f19202309` | **3/12 → 0/11** | 0 → 0 | **1/12 → 11/11** |
| `06647b9c2031` | **3/12 → 0/12** | **7 → 0** | 12/12 → 12/12 |
| `e78306202d78` | **1/2 → 0/2** | 0 → 0 | 2/2 → 2/2 |
| `ecfac1f3c0aa` | **1/3 → 0/3** | 0 → 0 | 3/3 → 3/3 |
| `3a3a96354546` | 0/0 → 0/0（材料块前后都是空的） | — | — |

**计划外的一条**：批 26 只读了判 0 率高的那两篇，所以报的是「两篇各 3/12」。
**四篇非空的全中**，而且按比例最坏的不是那两篇——`e78306202d78` 是 **1/2 = 50%**。

#### 那 78 格的新数：**没有，而且这一批拿不出来**

批 26 写的验法是「`cell_key` 带正文指纹，改完 `--report` 重跑，那 78 格自动重算」。
**`--report` 重算的是「哪些格子还算数」，不是那些格子的分数**——
分数是打分器给的，重算要重新发调用。实跑一次 `--report`（零调用）的结果：

| | 改之前 | 改之后 |
|---|---:|---:|
| 日志有效行 | 510 | **129** |
| `mismatched`（真作废） | 1053 | **1598** |
| 待跑格子 | 129 | **729** |
| `factual_grounding` 干净臂分母 | 159（批 26 报 156，差 3 格在前置条件那一档） | **33** |
| 判 0 | 75 = **47.2%** | 0 = 0.0% |

**剩下那 33 格只有 `3a3a96354546` 一篇**——它的材料块改前改后都是空的，
指纹没变，所以它是唯一没作废的。**「双峰没了」这句话这一批说不出口**：
双峰的那两篇，它们的格子现在一格都不在表里。

**重算的价签**：`--only note/whole/` = **336 格**，按 `llm_usage` 里
`bench/sensitivity` 那 2300 次调用的均值（prompt 2583 / completion 541）算
≈ **0.87M prompt + 0.18M completion token**，端点是
`api.openai.com` / `gpt-5.6-luna`（真金白银）。
**没跑，等指令**——任务书写的「零模型调用就能验」对「验修复生效」成立
（上面那张确定性对照表就是），对「拿到新的判 0 率」不成立。

---

### B —— 计划 2.4 接到 `section` + 六个 block（§5 第 5 行）

批 26 量出来的缺口：`gap_summary` 全仓唯一的读者是 `hooks/note.py:188`，
**8 个模式接了 1 个**；而实测 `note` 8.6%、block 模式 14–40%，
正落在批 13 隔离实验「只开 2.5 = 17.2%」那一档。

* **`section`**：`retrieval_plan_user(ledger_gaps=…)`，
  外加 `ledger_mw.note_topics(led, topics)` —— 主题树是 hook 直接调的、
  不进 `trace.calls`，`fold` 看不到它，不折分母的话缺口摘要里一条
  「一条都没取」都不会有（批 13 第一版就是这么漏的，`hooks/note.py`
  那条注释记着）。
* **六个 block**：`BlockHooks._user(gaps=…)`，摆在最前面。
  **只进 `prepare` 那一发，不进 `produce`**——它回答的是「还该查什么」，
  而写正文那一步问的是「怎么写」；把一份「你没有的材料」清单摆给写作看，
  换回来的是占位句（批 13 实测 `off` 篇 5 句占位）。默认参数是空串，
  `produce` 那一路一个字都不受影响，另有一条闸钉着。
* 开关照旧是 `params.LEDGER_IN_PROMPT` 一个，两条路各有一条「撤得掉」的闸。
* **`custom` 照接不误，但不算进验收**（批 26 ④：它的 `tool_calls` 合计 0，
  分母为 0）。接线六个一视同仁的理由是批 22 那条：*同一件事挡住一半等于没挡*
  ——少接一个，等它哪天有工具调用了又得回头找一遍。

#### block 模式接完之后的重复查询率：**没有新数**

**这一批零模型调用，`harness_rounds` 里 block 模式的行还是批 26 那批
（14–40%），一个字都没动。** `note` 那一侧当年是每臂 16 次跑量出来的，
block 这一侧要同样的证据就得同样跑一次。**接线不是成绩。**

---

### C —— 三列探针：`claim_atoms` / `fired_checks` / `abstained`

解锁 §5 第 8 行（候选原子率）和第 9 行（照做率，批 26 报「量不了，分母 0」）。

#### 「它的每个取值都真写得进去吗」——逐个验了

这条规矩来自批 22：`harness_runs.stopped` 从加进来那天起就记不到 `max_rounds`。
所以**落库之前先把取值列全，再逐个构造真能出现的那个局面**跑一遍
（`tests/test_round_probes.py`，一个取值一条闸，**不是往 bag 里塞值**，
而且五条都真的走一遍 `Checks.before_judge` → `Ledger.after_judge` 再把行读回来）。

| 列 | 取值 | 谁写得进去（闸里构造的就是这个局面） |
|---|---|---|
| `abstained` | `not_in_mode` | 六个 block 模式任一轮（`checks` 里没有 `unsupported_specifics`） |
| | `too_short` | 新写的不足 `MIN_FRESH_CHARS` 的轮（打磨轮 / 清理轮常态） |
| | `no_oracle` | 工具一次都没返回东西的轮 |
| | `no_atoms` | 写了一大段、里头一个完整日期 / 一个署名都没有 |
| | `judged` | 真的逐条比对过了 |
| | ~~空串~~ | **活着的跑一次都写不出它**——那是批 27 之前那些行的 `DEFAULT ''`。这件事明写在 `checks/claims.py` 的取值表里，免得下一个人把它当成一档 |
| `claim_atoms` | `-1` | 只跟 `not_in_mode` 同时出现 |
| | `0` | 判了、一个候选原子都没有 |
| | `≥1` | 写了完整日期或署名的轮；**弃权那几轮照样记**——「没有候选原子」和「没去看」是两件事，而第 8 行问的正是前者 |
| `fired_checks` | `[]` | 一条判据都没命中的轮 |
| | 一条 | 正常短路的轮 |
| | **两条** | `STUCK_ROUNDS` 卡死放行之后后面又命中一条——**这个取值就是它必须是数组的全部理由**，记成单值的话卡死那条从库里消失，而它恰恰是第 9 行最该看的一类 |

#### 两处窄化，各对着一条规矩

1. **探针算在判据循环之前，不算在 `unsupported_specifics` 里面。**
   循环是「第一条命中的赢、后面的不跑」——挂在那条判据里的话，只要有别的判据
   先短路，这一轮就什么都记不到，而**那正是第 9 行要看的那些轮**
   （「上一轮判据报了材料不够，下一轮照做没有」）。*同一件事挡住一半等于没挡。*
   突变验 C2 把那个 `if` 改成恒假，闸当场红。
2. **`-1` 跟 `0` 严格分开。** 记成同一个 0 的话，「这个模式压根不判」在库里就消失了
   ——批 22 的 `stopped` 就是这么坏的。
3. 另外：**当场被 `verdict.fix` 修好的那一条不算命中**。它对下一轮一个要求都没提，
   算进来是给第 9 行的分母灌一批没有要求的轮。
4. `probe()` 和 `unsupported_specifics` **共用同一段前置判断**（后者调前者），
   另有一条闸盯着判据里不许再出现 `MIN_FRESH_CHARS` / `has_tool_output`
   ——另写一份「跟判据一样的前置条件」，判据一改这一列量的就不是同一件事了。

#### 9.2 / 9.4 那三张空表：**C 一行都带不出来，说清楚**

批 26 让确认的三样，这一批复核仍然是 **`harness_edits` 0 行 / `harness_rounds.depth_dropped`
全 0 / `harness_snapshots` 0 行**。**C 加的是列，不是跑**——三列今天在库里
462 行上一律是迁移默认值（`-1` / `''` / `''`）。
`harness_edits` 要的是**用户真的编辑了 AI 写的东西**、`harness_snapshots` 要的是
**轮末暂停**，两样都不是加一列能变出来的。*光等是等不来的，得先有跑。*

---

### 突变验（22 条，**22 条变红——3 条第一版没抓住**）

| 突变 | 抓住的闸 |
|---|---|
| A1 切句退回「整篇一个串切」 | `test_整行就是排版的那几种真的剔掉` |
| A2 材料不去重 | `test_同一句话只占一个格子` |
| A3 量程退回整篇 | `test_材料只在打分器真的看得见的那段正文里摘` |
| A4 目录行不剔 | `test_材料里不许出现目录行本身` |
| A5 表格行和围栏内容整条剔掉 | `test_图块表块的材料不许是空的` |
| A6 图片 alt 不剔 | `test_整行就是排版的那几种真的剔掉`（**第一版没抓住，见下**） |
| A6b 图片整行剔掉（连同一行的散文） | `test_图片语法剔掉之后同一行的散文还在` |
| B1 `section` 不接账本摘要 | `test_账本摘要真的进了分段那条路的检索规划` |
| B2 `section` 不折主题树分母 | `test_分段那条路的主题树分母也在渲染摘要之前折进账本` |
| B3 block 不接账本摘要 | `test_六个block模式的取材那一发都带账本摘要`（六个模式逐个参数化） |
| B4 缺口摘要也塞进写正文那一发 | `test_缺口摘要只进取材那一发不进写正文那一发` |
| C1 `not_in_mode` 记成 0 | `test_取值not_in_mode_由没有这条判据的模式写进去` |
| C2 探针挂进判据里（别的判据先短路就记不到） | `test_取值no_atoms…` 那一串 |
| C3 `fired_checks` 退回单值 | `test_取值多条_由卡死放行之后又命中一条的轮写进去` |
| C4 修好的那条也算命中 | `test_当场被修好的那一条不算命中` |
| C5 / C5b 弃权就把候选数记成 0（两档各一条） | `test_候选原子数不因为弃权就记成0`（**第一版没抓住，见下**） |
| C6 三列不传给 `record_harness_round` | `test_五个取值真的各自落进harness_rounds` |
| C7 读完不 `pop` | `test_读完就pop_下一轮不会静默继承这一轮的数` |
| D1 脚本另抄一份分界那句话 | `test_材料里不许出现目录行本身` + 三条（**第一版的突变写错了，见下**） |

**三条第一版没红，先分清是哪一种**（铁律：先怀疑用例不够，但批 23 出过例外）：

* **A6「图片 alt 不剔」——用例不够。** 夹具里那段 alt 是纯文字、一个数都没有，
  于是它被「带日期/数字」那一关顺手滤掉了，**剔不剔图片在这份夹具上看不出差别**。
  真库里那条 alt 也没有数——所以这是「断言在别处被顺手满足」的又一例。
  给 alt 补一个数之后当场红，并补了反向那一条（剔的是 `![…](…)` 这一段，
  **不是「出现过图片的那一行」**：把带图的那句话整条丢掉等于少一条真材料）。
* **C5「弃权就把候选数记成 0」——用例不够。** 我只测了 `no_oracle` 那一档，
  而突变打在 `too_short` 那一档上。**弃权有两档，只测一档等于只挡住一半**；
  参数化成两档之后两个突变（C5 / C5b）都红。
* **D1 ——突变本身写错了，不是闸的问题。** 第一版只改 `score_context.VERBATIM_MARK`
  这个常量的值，而生产和脚本**都**从这个常量取——不红是**正确行为**，
  抽常量要的就是这个。重写成「生产改措辞 + 脚本硬编旧字面」（也就是真的抄一份）
  之后，4 条闸变红。*一个改完之后行为本该不变的改动，不是突变。*

---

### 闸

后端 **1933 passed**（基线 1888 + 本批 45 条：bench 切句 **14**、`section` **3**、
block **8**、三列探针 **20**）。前端 **52 个文件 263 条**，一条没动。

`harness-framework.md` §21 补两条规矩（「落库加一列之后要问它的每个取值都真写得进去吗」、
「一个可能为空的量程就不是量程」）。**Mode / 工具 / check / middleware 四个数一个没动**
（`claims.probe` 不是 check，`fact_lines` / `verbatim_part` 在 bench 里），
`test_doc_counts` / `test_directory_map` / `test_dimension_method_gates` /
`test_architecture_claims` 照绿。

`docs/harness-upgrade-plan.md`：§5 第 5 / 8 / 9 / 10 四行改写，
阶段 2.4 补接线记录，阶段 12 加 12.5（4/4 → 5/5）。

---

### 计划外发现（五条）

1. **结构残骸不是那两篇独有的，四篇非空的全中，而且最坏的不是那两篇。**
   批 26 只逐条读了判 0 率高的两篇（各 3/12），所以结论写成了「markdown 重的笔记」。
   实际 `e78306202d78` 是 **1/2 = 50%**、`ecfac1f3c0aa` 是 1/3 = 33%。
   *读出来的比例，分母是「读了几篇」不是「有几篇」。*
2. **48.1% 是两条根因叠出来的，而批 26 的措辞把它归成了一条。**
   `06647b9c2031`（94.1%）只中切句那条——它 2762 字，整篇逐字给打分器，
   12 条材料原句 12 条都在 body 里；`309f19202309`（100%）两条全中，
   而且量程那条更致命（12 条里只有 1 条看得见）。
   **两篇的双峰是同一个数字下的两种病。**
3. **一条批 8 留下的老闸抓住了这一批的第一版设计错误**（`test_真正发出去的那一格带的
   就是生产那份上下文`）。它当年是为「`build_tasks` 到底用没用 `production_context`」
   写的，今天挡住的是「材料被剔空了」。*一条钉住「真正发出去的是什么」的闸，
   挡得住它当年没想到的事。*
4. **`06647b9c2031` 的围栏线是奇数（3 条）。** 这是台账第 1 行那篇机器损伤笔记的
   又一处形态——开头那条围栏的「开」在损伤里丢了。它逼出了「不需要状态机的地方
   不要状态机」那个改法；顺带说明**真库上的畸形结构会稳定地绊倒任何一个假设
   markdown 合法的解析器**。
...
碰真库的每一处都只读、夹在 `db_guard.Watch()` 里、**命令不经过任何管道**、
单独打 `EXIT=$?`。**没有读也没有写 `~/Library/Application Support` 那一份**（62 篇那个库）。
`.local/criteria_drift/latest.json` **没有被碰过**（这一批一次都没跑 `criteria_drift.py`）。

### 这一批的实际成本

**模型调用 0 次、0 token。** 全部验证在：
`--report`（读 1785 行日志，零调用）、`derived_facts` 的确定性前后对照
（五篇真实笔记，只读）、22 条突变验、全量闸 4 次。

### 下一步（排序 + 理由）

1. **把那 336 格跑回来**（`--only note/whole/ --repeats 3`，≈1.0M token）。
   **A 修完的唯一一个还没回答的问题就是它**：48.1% 的双峰在修好的材料上还剩多少。
   这一条要花钱，**先说成本再跑**。
2. **B 的效果验证：block 模式各跑几次。** 批 13 那次是每臂 16 次跑；
   block 模式一次跑比 note 短（`max_rounds` 3 对 8），可以只跑 `eda` / `table`
   （今天 34.1% / 40.0%，最高的两个）。**`custom` 不用跑，分母是 0。**
3. **让三列长出行来。** 第 1、2 条任何一条跑起来，`claim_atoms` / `fired_checks` /
   `abstained` 就有数了——它们记在生产链路上，不是 bench 里。
   **顺序上它排第 3，是因为它搭在前两条的车上，不用单独花钱。**
4. **第 13 行 `material_use`。** 批 26 排第 4 的理由（「它可能是第 1 条的下游」）
   这一批坐实了一半：材料确实摘坏了，而且坏法之一是**同一句话占了 8 个格子**
   ——「材料全剔光」和「材料没剔光」在那份材料上本来就差不多。
   等第 1 条跑完再看它还剩多少问题。
5. **第 6 行的目标改掉**（不是改代码，是改这张表）。批 26 已经三条依据说死了，
   留着一个证明达不到的目标，下一个人会去追它。

**明确不做的**（批 26 划的两条，这一批照旧）：不给 `topic_fidelity` 补位置级探测器
（分母一次都没量过）；不因为 `right_kind` 的 p=0.1025 去改那条已经修好的判据
（n=1 篇 × 3 次时两侧 p 最小就是 0.10，那是数学不是判据）。

---

## 批 28 · 把批 27 的三条接线跑成数（2026-09-18）

开工 HEAD `aa0af48`。**这一批是三批以来第一次真发调用**：A 把批 27 作废的
那 336 格跑回来、B 让 block 模式接上 2.4 之后各跑 6 次、C 顺着 B 的车把三列探针读回来。
**一行产品代码没改**，改的只有三份文档。全程 `db_guard.Watch()`、命令不经管道、
单独打 `EXIT=$?`、分段跑（每段 360 秒预算，两段跑完）。

**一句话结论**：**双峰没塌，而且长出了第三个峰**——`06647b9c2031` **97.6%**、
`309f19202309` **82.1%**、`e78306202d78` **0.0% → 42.2%**，另两篇仍精确 0.0%。
批 27 那句「打分器判 0 是它判对了，坏的是这份材料」**只说对了一半**：材料修好之后
打分器照样判 0，而且逐格读下来三篇判 0 的**理由各不相同**（见下）。

---

### A —— 那 336 格的新数：**48.1% → 48.7%，双峰原地不动**

`--only note/whole/ --repeats 3`，16 条 probe × 5 篇 × 3 次 × 2 臂 = 384 格，
其中 48 格（`3a3a96354546`，材料块前后都是空的、指纹没变）批 27 之后仍有效，**本批真跑 336 格，0 格出错**。
口径照批 26：`classify_stale → used`、前置条件成立、clean 臂、`scores` 里有 `factual_grounding`
——**189 格**（192 − 3 格 `3a3a96354546/strip_specifics` 前置条件没成立）。

| 笔记 | 批 26（旧材料） | **批 28（新材料）** | 判 0 的形状（逐格读出来的） |
|---|---:|---:|---|
| `06647b9c2031`（2762 字） | 48/51 = 94.1% | **41/42 = 97.6%** | **42/42 格 ≥3 维同时为 0**：`non_repetition` 均值 **0.00**、`coherence` **0.00**、`style_fit` 0.10、`factual_grounding` 0.02。打分器是在给**整篇**判死刑，不是在判事实——这篇正是 §5 第 1 行那篇机器损伤笔记（段内重复 36.4%、从词中间接上、结尾「 人验证标准漂移。」这种断句） |
| `309f19202309`（30588 字） | 27/27 = 100% | **32/39 = 82.1%** | 按档拆：`as-deployed` **15/21 = 71.4%**、`claims` 5/6、**`whole-piece` 12/12 = 100%**。8/39 格 ≥3 维为 0。`whole-piece` 递的是整篇 30588 字，材料上限 12 条、全落在前 21%——**材料修好了，量程那条在 `whole-piece` 档上照样成立**（那一档按设计就是整篇） |
| `e78306202d78`（1976 字） | 0/6 = 0.0% | **19/45 = 42.2%** | **0/45 格 ≥3 维为 0**，其他维正常（`spine_fidelity` 2.00、`material_use` 2.00）——这才是一次**真的**「材料撑不住正文」：材料只摘得出 **2 条**（批 27 修完 2/2 都是干净散文），正文却满是 Speaker A / Speaker B / Discord / MemoCad 这类材料里没有的具体物。批 26 的 0.0% 分母是 **6 格**，今天是 45 格 |
| `ecfac1f3c0aa`（1622 字） | 0/42 = 0.0% | **0/36 = 0.0%** | `factual_grounding` 均值 1.39（分值 1 有 22 格、2 有 14 格）。材料 3 条恰好盖住正文里三个带日期的句子 |
| `3a3a96354546`（817 字） | 0/30 = 0.0% | **0/27 = 0.0%** | 材料块空，判据按「没给材料就算达标」，均值 1.96 |
| **合计** | **75/156 = 48.1%** | **92/189 = 48.7%** | 按档：`as-deployed` 49/105 = 46.7%、`claims` 13/27 = 48.1%、`whole-piece` 28/51 = 54.9%、`with-evidence` 2/6 |

**分母不是同一批格子**：批 26 的 156 格是旧材料指纹下、混着几个 `--repeats` 跑出来的；
批 28 的 189 格全是本批 `--repeats 3` 一次跑齐的。**能比的是形状，不是逐格**。
`--repeats 2` 重算一遍（丢掉 rep=2）形状一样：63/126、`06647` 27/28、`309f` 22/26、`e783` 14/30、另两篇 0。

#### 逐条读了什么（零调用，`build_tasks` 把每格递给打分器的东西原样取出来）

1. **`06647b9c2031` 的材料块 12 条全是干净散文**（F1「成功标准：10人小范围可对外验证…」…
   F12「知识库同时记录 EVT 为 4 月 10 号启动…」），批 27 的修法在这篇上确实生效。
   但同一格的分数是 `{spine 1, beat 2, non_repetition 0, factual 0, coherence 0, material 2, style 0}`
   ——打分器看到的正文头是「⏎## APP定义…」、尾是「…轻量匹配界面。⏎⏎ 人验证标准漂移。」，
   中间「并行介入，完成跨境数据隔离方案初稿」从半句起头。**它判的是这篇坏了**。
   **修材料修不动这一篇**，因为它判 0 的理由不在材料里。
2. **`309f19202309` `as-deployed` 那一格**：body 4546 字 = 24 行目录 + 最后 3 节，
   材料 11 条全部来自这 3 节（批 27 的量程修复逐字可见），这一格打了 `factual 1`。
   而 `whole-piece` 同一篇：body 30588 字，材料 12 条全在「公司汇报」开头那一节
   （F1「去年公司稳健发展…」到 F12「25 万路摄像头…」），恒瑞医药那三节一条材料都没有
   →打分器 12/12 判 0。**这一档的判 0 是材料上限 12 条 vs 三万字正文的结构性结果**，
   不是切句、也不是量程——是「12 条」这个上限本身。
3. **`e78306202d78`**：材料 2 条（F1「午餐会要把方向共识转化为可落地的闭环方案…」、
   F2「硬件交互路径已定…赶不上 6 月末/7 月销售窗口」），正文 1976 字里 Speaker A/B、
   Discord、MemoCad、「录完之后他还是要打开手机，传啊」全都不在材料里。
   同一格 `as-deployed` 打 1、`whole-piece` 打 2——**打分器在这篇上是摇摆的**，
   45 格里 0 有 19、1 有 16、2 有 10。
4. **代码判据（`claims.unsupported_specifics`）在 189 格干净臂上一次都没开火**
   （`claims` 档 clean 臂 0 格短路；植入臂短路 18 格全是真植入）。用它的原子函数
   零调用数一遍「材料盖不住的原子」：`ecfac1f3c0aa` **3 个**（三个 `2026-03-31` 式日期）
   判 0 率 0.0%，`06647b9c2031` **1 个**（Speaker D）判 0 率 97.6%。
   **模型的判 0 跟代码可数的原子不相关**——它看的不是原子。

#### bench 自己那张表跟着变了两处（**计划外**）

* **§5 第 13 行 `material_use` 对「材料全剔光」：2.0 → 1.67（p=0.51）变成 2.0 → 0.75，掉 1.25，p=0.0002，「抓住」**。
  批 26 排第 4 时写的「它可能是第 1 条的下游」这一批坐实：材料里同一句占 8 格的时候，
  剔光和没剔光在打分器眼里本来就差不多。**没改判据，只改了材料，这一行就从「只动了一点」变成「抓住」。**
* `factual_grounding` 的五条 probe **全部落进「基线偏低」**（clean 均值 0.5–0.87）：
  bench 的规矩是干净版本身低于线就不从「植入后没掉」推结论，于是**这一维的灵敏度在这份语料上量不了**
  ——批 26 那次 `fabricate_specifics·as-deployed` 1.22 → 0.22「抓住」是在旧材料上的，今天不成立。
  噪声标定线从 1.33（7 行）落到 **0.42**（16 行）。
* `material_use` 满分率仍 **189/189 = 100%**（§5 第 12 行那一半没动）。

---

### B —— block 模式接上 2.4 之后：**`table` 40.0% → 0.0%，`eda` 34.1% → 32.5%**

`block_rounds_probe.py --only eda --repeats 6` / `--only table --repeats 6`，
种子跟批 26 那批完全一样（`309f19202309` 光标 1000，`rails_off=("save",)`，chart 的文生图排掉）。

| 模式 | 接线前（批 26，09-18 06:20–07:05） | **接线后（批 28）** | 分母 |
|---|---:|---:|---|
| `table` | 4/10 = **40.0%** | **0/10 = 0.0%** | 各 6 次跑 / 10 轮 / 10 次调用 |
| `eda` | 29/85 = **34.1%** | **26/80 = 32.5%** | 6 次跑 / 18 轮 / 85 次 → 6 次跑 / 17 轮 / 80 次 |

`eda` 逐轮读：第 1 轮 6 次全是 0 重复，第 2/3 轮每轮 2–5 次重复（调用 3–6 次）——**11 个后续轮 26 次，每轮 ≈2.4 次**。
**为什么 2.4 对 `eda` 没用（读代码得出的解释，没读到调用本身——账本不落库）**：
`gap_summary` 只列 `query_cache.CACHEABLE` 那八个读知识库的工具（`memory` 组），而 `ledger.fold` 的
`repeat_calls` 数的是**全部**工具；`eda` 的 task 第 1 步逐字写着「**先 numbers_near_cursor**…再 list_tables」，
这两个都在 `data` 组、不在 CACHEABLE，每一轮都会照做一遍——第 2/3 轮起它们必然是「参数完全相同的重复」，
11 个后续轮 × 2 = 22，跟 26 对得上。`table` 的 4 次重复消失了，跟「它重复的是能被摘要列出来的那类」一致。
**这条解释要坐实需要一次带 trace 的 `eda` 跑（≈60k token），本批没跑。**
如果坐实，§5 第 5 行 `eda` 那个数量的是「模式自己要求的重取」，**不是 2.4 该管的重复**——
*一个分母里混着两种东西，量出来的比例谁也不代表*（批 26 的第 6 条规矩换了个形状）。

另一条顺手看见的：`harness_runs` 108 → 120，**`eda` 第一次在 `harness_runs` 里有行**
（此前 6 次 `eda` 跑一行都没有——它们跑在 06:21–07:03，早于批 22 修 `History` 的那次落地 09:56）。
6 行 `stopped`：`nothing_changed` 3 / `max_rounds` 3；`table` 6 行全 `complete`。

---

### C —— 三列探针读回来：**27 行新行，五个取值出现了一个**

| 列 | 出现的取值（27 行新行） | 没出现的 | 为什么出现不了 |
|---|---|---|---|
| `abstained` | `not_in_mode` × 27 | `too_short` / `no_oracle` / `no_atoms` / `judged` | **本批真跑的只有 block 模式**。A 走的是 bench 的 `evaluate()`，从不经过 `loop.py`，一行 `harness_rounds` 都不写。那四个取值要 `note` / `section` 跑才写得出来（一次 `note` 跑中位 116k token） |
| `claim_atoms` | `-1` × 27 | `0` / `≥1` | 同上——`-1` 正是「这个模式没有那条判据」，六个 block 模式按设计只写它 |
| `fired_checks` | `[]` 24 · `["charts_from_tools"]` 2 · `["table_present"]` 1 | 两条的数组 | 需要 `STUCK_ROUNDS` 卡死放行之后再命中一条，17 轮 `eda` 里没出现 |

**这不是批 22 `stopped` 那种「记不到」**：`not_in_mode` / `-1` / 单条数组三种都真写进去了，
而且跟模式对得上（`eda` 17 行 + `table` 10 行全是 `not_in_mode`，`table_present` 只在 `table` 出现）。
剩下的取值是**没跑到**，不是**写不进**——批 27 的闸逐个构造过。**三张空表复核**：`harness_edits` **0 行**、
`harness_rounds.depth_dropped > 0` **0/489**、`harness_snapshots` **0 行**（跟批 26 / 27 一样）。

---

### 突变验（量法的，不是判据的——这一批没改判据）

A 的量法把每道过滤逐个撤掉重算，**变的才是在挡东西的**：

| 突变 | 合计 | 结论 |
|---|---:|---|
| M0 正式口径 | 92/189 | — |
| M1 撤前置条件过滤 | 92/192 | 只多 `3a3a96354546` 的 3 格，方向不变 |
| M2 撤指纹过滤（532 行作废行混进来） | **191/458**，还冒出两篇已不在语料里的 `574f4f` 5/56、`603dca` 0/30 | **指纹过滤是承重的**——不过滤等于把旧材料的数拼进新表（批 7 的坑） |
| M3 全撤 | 192/398 | 同上 |
| M4 `--repeats 2` | 63/126，五篇形状不变 | 结论不依赖第 3 次重复 |

B 的自校验：同一条 SQL 在旧行上逐字复现批 26 的 29/85、4/10，再算新行。

### 闸

后端 **1933 passed**（基线 1933，一条没动）、前端 **52 个文件 263 条**（一条没动）。
`harness-framework.md` §21 补一行（「判 0 率要连同格其他维一起读」）。Mode / 工具 / check / middleware 四个数一个没动。
`docs/harness-upgrade-plan.md` §5 第 5 / 8 / 9 / 10 / 13 五行换成真数。

### 计划外发现（五条）

1. **批 27「打分器判 0 是它判对了」只对 `e78306202d78` 成立。** 另两个峰各有各的病：
   `06647b9c2031` 是打分器给整篇判死刑（七维里四维全 0），`309f19202309` `whole-piece` 是
   12 条材料上限对三万字正文。**同一个「判 0 率」下面是三种不同的东西，修材料只动得了其中一种。**
2. **`e78306202d78` 从 0.0% 变成 42.2%，是因为批 26 的 0.0% 分母只有 6 格。**
   *读出来的比例，分母是「读了几篇」不是「有几篇」*——这次是「几格」。
3. **§5 第 13 行不改判据就从「只动了一点」变成「抓住」**。上游材料坏了，下游判据看着像没牙。
4. **`eda` 的重复查询率量的可能不是 2.4 该管的东西**（分母混着 `data` 组工具），待一次带 trace 的跑坐实。
5. **成本估算错了 1.85 倍**（见下）——按全 bench 的均值 2583 估 note/whole 这一档，而这一档里
   `309f19202309` 的 `whole-piece` 格每次都递整篇 30588 字。**估算要按要跑的那些格子算，不是按历史均值。**

### 开工前后的 `notes` 指纹（自己核对过，不是自报；每段跑完各核一次，共 4 次）
...
**没有读也没有写 `~/Library/Application Support` 那一份**（62 篇那个库）。
`.local/criteria_drift/latest.json` 没碰。

### 这一批的实际成本（`llm_usage` id > 5283，端点 `gpt-5.6-luna`）

| | 调用 | prompt | 其中 cached | completion |
|---|---:|---:|---:|---:|
| A bench 336 格 | 321（15 格 `claims` 档代码短路，0 调用） | **1,724,868** | 1,435,543（83%） | 214,485 |
| B `eda` 6 跑 | 83 | 313,198 | — | 23,911 |
| B `table` 6 跑 | 39 | 88,814 | — | 6,724 |
| **合计** | **443** | **2,126,880** | 1,723,996（72.6%） | **245,120** |

**批 27 估的是 A ≈ 0.87M prompt + 0.18M completion = 1.05M；实际 A = 1.94M，1.85 倍。**
前 160 格（`309f19202309` + `06647b9c2031`）就用掉 1.20M prompt——估算用的是全 bench 的均值，
而这一档的 `whole-piece` 格每次都递 30588 字。B 没有事先估（批 27 只写「各跑几次」），实际 0.43M。

### 下一步（排序 + 理由）

1. **把 `06647b9c2031` 从判 0 率的分母里拿出来单独成表。** 它是机器损伤笔记（§5 第 1 行），
   打分器在它上面七维四维全 0，任何判据的「干净臂」在它上面都不干净。**不是修判据，是修语料口径**
   （`corpus_lineage` 加一档「损伤」，或 bench 的 `MIN_CHARS` 旁边加一条 `restated_ratio` 上限）。零调用可验。
2. **`whole-piece` 档的材料上限 12 条对三万字正文是结构性的**——要么材料按正文长度放宽，
   要么这一档只在 `context_keep_last` 以内的笔记上跑。先定口径再花钱。
3. **一次带 trace 的 `eda` 跑**（≈60k token），坐实「26 次重复里 22 次是 `numbers_near_cursor` / `list_tables`」。
   坐实之后 §5 第 5 行的分母要拆成「2.4 管得着的」和「模式自己要求的」。
4. **让另外四个 `abstained` 取值长出来**：一次 `note` 跑（≈116k token）就够。
5. bench 的日志加一列打分器的逐维 message（纯代码，零调用）——这一批想读判 0 的理由只能靠旁证，
   因为 `run_one` 把 `ev.scores[k].message` 丢掉了。
