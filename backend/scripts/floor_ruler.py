...
REGISTRY_SIZE_FLOOR = 125  # 第 819 轮 P98 在自己 worktree 里实测（+4 条：换门那一组读数）；818 轮是 121、817 轮是 114、816 轮是 104、814 轮是 95、813 轮是 85
# ⚠️ 合并时记得抬到**合并后**那个数：两批各自在自己 worktree 里抬到 84 / 79，
# 合并后真值是 85——取任一边都会让「删掉一条登记」不红（这个数只准往上，抬是绿的）。
CHECKED_COUNT_FLOOR = 137  # 同上；共核 = 登记表 + 例反例；818 轮是 133、817 轮是 126、816 轮是 116、814 轮是 107、813 轮是 97、811 轮是 90
